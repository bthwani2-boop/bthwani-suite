...
| `DSH-RUN-P014-02` | app-client cart + checkout intent + checkout failure | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx` | `runtime-proven` | `BLOCKED_WITH_REASON` | `API_CLIENT_BOUND__NEEDS_RUNTIME_EVIDENCE` | `BLOCKED_WITH_REASON` | pending real auth-service verification and WLT E2E payment callback runtime | test local-preview fallback and run local smoke test |
