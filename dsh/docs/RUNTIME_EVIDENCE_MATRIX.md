...
| `DSH-RUN-P014-02` | app-client cart + checkout intent + checkout failure | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx`; `auth-service` | `runtime-proven` | `PASS` | `api-db-runtime-proven__transport-bound__screen-runtime-proven` | `PASS` | none — E2E checkout / payment / callback integration verified in `tools/registry/runs/DSH_JOURNEY_003_AUTH_CLIENT_BINDING_EXECUTION-20260604/03-verification.txt` | none |
| `DSH-RUN-P014-03` | app-client tracking + support | `DshOrdersListScreen.tsx`; `DshTrackingScreen.tsx`; `DshConversationHubScreen.tsx`; `DshOrderIssueHubScreen.tsx`; `DshProxyHubScreen.tsx` | `runtime-proven` | `PASS` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `PASS` | none — E2E orders list and real-time tracking with WebSocket push updates verified | none |
...
| `DSH-RUN-P014-05` | app-captain pickup + delivery + PoD | `DshCaptainOrdersScreen.tsx`; `DshCaptainPickupDropoffScreen.tsx`; `DshCaptainPoDSubmissionScreen.tsx`; `DshCaptainMapScreen.tsx` | `runtime-proven` | `PASS` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `PASS` | none — captain assignment, pickup dropoff, and proof of delivery submission verified | none |
...
| `DSH-RUN-P014-07` | control-panel operations | `operations.registry.ts`; `CommandCenterScreen.tsx`; `DispatchAssignmentScreen.tsx`; `ExceptionsEscalationsScreen.tsx`; `AuditSupportSlaScreen.tsx`; `GeoHeatmapScreen.tsx` | `runtime-proven` | `PASS` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `PASS` | none — command center, live orders, geo-heatmap and exceptions management verified | none |
| `DSH-RUN-P014-08` | control-panel finance bridge | `FinanceHubScreen.tsx`; `FinanceHubScreens.tsx`; `WltBoundaryBanner.tsx` | `read-only bridge`, `postgres` | `PASS` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `PASS` | none — read-only wallet summary rendering and WLT-owned finance bridge verified | none |
