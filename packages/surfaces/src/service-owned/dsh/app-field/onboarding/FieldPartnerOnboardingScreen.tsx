...
import {
  ...
  type FieldStoreFile,
} from '../stores/fieldStoreModel';

type FieldPartnerOnboardingScreenProps = {
...
export function FieldPartnerOnboardingScreen({ store, onBack, onStoreChange, onSaveDraft, onSubmitReview }: FieldPartnerOnboardingScreenProps) {
  ...
  return (
    <Box style={{ flex: 1 }} background="background">
      ...
      <StickyActionBar
        note={readOnly ? 'الملف في حالة للقراءة فقط الآن. بقي للميداني السجل والحالة والعمولة فقط.' : missingItems.length ? `النواقص الحالية: ${missingItems.join('، ')}` : 'كل الأساسيات مكتملة. يمكنك الإرسال للمراجعة.'}
        primaryAction={{
...
export default FieldPartnerOnboardingScreen;