    ...
    if (activeTab === 'catalog') {
      actions.push(
        ...
        },
        {
          ...
          isActive: false,
          onAction: () => {
            const proposal = makeProposal({ type: 'media-policy-change', label: 'تبديل سياسة صور المجموعة', note: 'مقترح معاينة | تبديل سياسة الصور للمنتجات المحددة في الجدول', productIds: filteredProducts.map(f => f.id) });
      ...
      actions.push({
        ...
        isActive: false,
        onAction: () => {
          const proposal = makeProposal({ type: 'bulk-approve', label: 'اعتماد مقترحات الشركاء', note: 'مقترح معاينة | اعتماد مقترحات الشركاء المحددة ونقلها لمرحلة الجاهزية', productIds: filteredProducts.filter(p => p.approvalStage === 'partner-proposed' || p.sourceSurface === 'partner').map(f => f.id) });
          pushPreviewProposal(proposal);
          setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
        },
      });
      ...
      if (activeSubTab === 'marketing') {
        actions.push(
          {
            ...
            onAction: () => {
              const proposal = makeProposal({ type: 'bulk-approve', label: 'اعتماد كل مراجعات التسويق', note: 'مقترح معاينة | اعتماد مراجعات التسويق المحددة بنجاح', productIds: filteredProducts.filter(p => p.approvalStage === 'marketing-review').map(f => f.id) });
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'bulk-approve', label: 'تمرير جميع فحوصات الجودة', note: 'مقترح معاينة | تمرير فحوصات الجودة لمنتجات الشركاء بنجاح', productIds: filteredProducts.filter(p => p.approvalStage === 'partner-review').map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'price-change', label: 'تسوية تعارض الأسعار تلقائياً', note: 'مقترح معاينة | خفض وتعديل الأسعار المرتفعة وتسوية تعارض التسعير', productIds: filteredProducts.filter(p => p.price > 100).map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'media-policy-change', label: 'تعيين صور مركزية معتمدة', note: 'مقترح معاينة | تعيين صورة مركزية افتراضية للمنتجات التي تنقصها صور', productIds: filteredProducts.filter(p => !p.mediaKey).map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
      ...
      if (activeSubTab === 'categories') {
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'conflict-resolution', label: 'دمج وحل جميع التكرارات', note: 'مقترح معاينة | دمج التكرارات وحل النزاعات للمنتجات المحددة', productIds: filteredProducts.map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'edit-product', label: 'مزامنة الباركود مع المعرف', note: 'مقترح معاينة | تعيين GTIN بالاعتماد على SKU للمنتجات المحددة', productIds: filteredProducts.filter(p => !p.gtin).map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
      } else if (activeSubTab === 'media') {
        actions.push({ id: 'ma-media-policy-strict', label: '📸 فرض سياسة الصور المركزية', isActive: false, onAction: () => setActionMessage('preview-only: تم فرض سياسة الصور المركزية للمنتجات المؤهلة') });
        ...
        actions.push({
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'visibility-change', label: 'تبديل الظهور للمستهلكين', note: 'مقترح معاينة | تعديل منصات العرض المتاحة للمنتجات المحددة', productIds: filteredProducts.map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        });
      }
      ...
      actions.push(
        ...
        },
        {
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'visibility-change', label: 'نشر جميع المنتجات الجاهزة للعميل', note: 'مقترح معاينة | نشر جميع المنتجات الجاهزة بنجاح للعميل', productIds: filteredProducts.filter(p => p.approvalStage === 'catalog-adopted').map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        },
        {
          ...
          onAction: () => {
            const proposal = makeProposal({ type: 'visibility-change', label: 'إخفاء جميع المسودات والمقترحات', note: 'مقترح معاينة | التأكد من إخفاء جميع المسودات ومقترحات الشركاء', productIds: filteredProducts.filter(p => p.approvalStage === 'catalog-draft' || p.approvalStage === 'partner-proposed').map(f => f.id) });
            pushPreviewProposal(proposal); setActionMessage(`📋 مقترح: ${proposal.label} (${proposal.status})`);
          },
        }
      );
