...
New canonical human-review folder: dsh/docs/journies-011-performance-cleanup-refactor/
...
# J-011 — Performance, Cleanup, Refactor, Maintainability
...
|---|---|
| Journey ID | `J-011` |
| Name | Performance, Cleanup, Refactor, Maintainability |
| Type | `foundation` |
| Primary Actors | developer |
| Control Panel / Domain Owner | architecture/performance |
| WLT Boundary | none except finance code ownership scan |
| Planning Status | `REQUIRED_BEFORE_FINAL_CLOSURE` |
| Business Outcome | Heavy screens/files are decomposed, duplicated/dead code removed, data loading is scoped, APIs avoid N+1, and runtime remains fast. |
...
|---|---|---|---|---|---|---|
| `DSH-SLICE-011A` | DshClientSurface Decomposition Plan | developer | app-client surface | split huge surface into maintainable units without breaking flow | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011A-dshclientsurface-decomposition-plan.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-011B` | Backend Repository Interface Split | developer | Go backend | split oversized repository contracts by domain | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011B-backend-repository-interface-split.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-011C` | N+1 Media and Product Query Prevention | developer | backend + client adapters | batch media/products and avoid per-card requests | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011C-n-1-media-and-product-query-prevention.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-011D` | Polling Backoff WebSocket Strategy | developer | tracking/runtime | replace fixed heavy polling with backoff or event strategy | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011D-polling-backoff-websocket-strategy.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-011E` | Dead Code Duplication Noise Scan | developer | repo targeted scan | remove/treat duplicates/dead code/scattering with rollback | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011E-dead-code-duplication-noise-scan.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-011F` | Index and Query Performance Gate | developer | database/migrations | prove indexes for filter/search/status fields | `slices/j-011-performance-cleanup-refactor-maintainability/DSH-SLICE-011F-index-and-query-performance-gate.md` | NOT_CLOSED_BY_THIS_FILE |
