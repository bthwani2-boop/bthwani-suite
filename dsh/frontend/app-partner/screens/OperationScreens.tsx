  ...
  return (
    <Box gap={4}>
      <OperationHeader
        ...
        chips={
          <>
            <Chip label={`الحالة الحالية: ${auctionStatusItems.find((item) => item.id === selectedStatus)?.title ?? ''}`} tone="brand" selected />
            <Chip label={`المرحلة التالية: ${nextStatus ? auctionStatusItems.find((item) => item.id === nextStatus)?.title : 'إغلاق نهائي'}`} tone="info" />
          </>
        }
      />
