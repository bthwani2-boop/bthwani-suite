...
export type DshCustomerStateGroup =
  | 'quote'
  | 'serviceability'
  | 'store'
  | 'cart'
...
export type DshCustomerStateVisibility = {
...
export type DshCustomerStateMeta = {
  state: DshCustomerState;
  group: DshCustomerStateGroup;
  label: string;
  title: string;
  description: string;
  visibility: DshCustomerStateVisibility;
  isTerminal: boolean;
  isException: boolean;
  walletVisible: boolean;
};

const hiddenVisibility: DshCustomerStateVisibility = {
  quoteVisible: false,
...
const customerStateMeta: Record<DshCustomerState, DshCustomerStateMeta> = {
  quote: {
    state: 'quote',
    group: 'quote',
    label: 'تجهيز التسعير',
...
export function getDshCustomerStateMeta(state: DshCustomerState): DshCustomerStateMeta {
  return customerStateMeta[state];
}

export function isDshCustomerTerminalState(state: DshCustomerState): boolean {
  return customerStateMeta[state].isTerminal;
}

export function isDshCustomerExceptionState(state: DshCustomerState): boolean {
  return customerStateMeta[state].isException;
}

export function isDshCustomerWalletVisibleState(state: DshCustomerState): boolean {
  return customerStateMeta[state].walletVisible;
}
