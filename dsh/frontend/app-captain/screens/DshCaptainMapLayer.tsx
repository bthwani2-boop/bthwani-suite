...
import type { MapHeatZone } from '../../shared/delivery/captain/captain.contract';
...
export function DshCaptainMapLayer({
  ...
  const { Switch: RNSwitch } = React.useMemo(() => {
    try {
      // eslint-disable-next-line no-eval
      return { Switch: eval('require')('react-native').Switch };
    } catch {
      return { Switch: null };
    }
  }, []);
      ...
      <Surface tone="inset" padding={0} gap={0} radiusToken="xl" style={{ flex: 1, overflow: 'hidden', borderColor: theme.lineStrong }}>
        <Box style={{ flex: 1, backgroundColor: theme.surfaceSecondary, overflow: 'hidden' }}>
          ...
          <Box style={{ position: 'absolute', left: 8, right: 8, top: 4, zIndex: 9999, elevation: 20 }}>
            <Surface tone="inset" padding={1} gap={1} radiusToken="xl" style={{ alignSelf: 'stretch' }}>
