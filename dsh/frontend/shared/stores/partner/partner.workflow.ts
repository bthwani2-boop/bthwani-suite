// Moved from state-machines/workflow.ts — canonical location is partner/partner.workflow.ts
...
export const dshPartnerIntakeMetrics: ReadonlyArray<DshPartnerIntakeMetric> = [
  { id: 'metric-partner-pending', label: 'المراجعة الأولية', value: 4, description: 'عناصر وصلت من الميداني أو الشريك وتنتظر الموافقة الأولى.' },
  { id: 'metric-marketing-pending', label: 'المراجعة التسويقية', value: 2, description: 'بطاقات مرّت على الشركاء وتنتظر الموافقة التسويقية.' },
  { id: 'metric-live-catalog', label: 'الكتالوج المباشر', value: 28, description: 'كل ما نُشر بالفعل وأصبح ظاهرًا لكل الشركاء.' },
];

export const dshPartnerIntakeItems: ReadonlyArray<DshPartnerIntakeItem> = [
  { id: 'product-olive-oil', productName: 'زيت زيتون بكر ممتاز', categoryLabel: 'المقاضي', source: 'app-field', stage: 'pending-partner', ownerLabel: 'الميداني', note: 'أضيف من الحقل ويحتاج فقط للمراجعة الأولى قبل التحويل.', submittedAt: 'اليوم 09:40' },
  { id: 'product-coffee-beans', productName: 'حبوب قهوة مختصة', categoryLabel: 'المقاهي', source: 'app-partner', stage: 'pending-partner', ownerLabel: 'الشريك', note: 'منتج جديد أرسله الشريك مباشرة على بوابة الإضافة.', submittedAt: 'اليوم 10:05' },
  { id: 'product-cookie-box', productName: 'صندوق كوكيز موسمية', categoryLabel: 'المطاعم', source: 'app-partner', stage: 'pending-marketing', ownerLabel: 'الشريك', note: 'اجتاز المراجعة الأولية ويحتاج ضبطًا تسويقيًا قبل النشر.', submittedAt: 'اليوم 11:15' },
  { id: 'product-published-meal', productName: 'وجبة جاهزة عائلية', categoryLabel: 'المطاعم', source: 'app-field', stage: 'published', ownerLabel: 'الكتالوج', note: 'منتج منشور بالكامل ويظهر الآن لكل الشركاء.', submittedAt: 'أمس 07:55' },
];

export const dshPartnerApprovalLanes = [
  { id: 'lane-field', title: 'الوارد من الميداني', description: 'الطلب يدخل من الحقل ثم يمر على بوابة الشركاء أولًا.' },
  { id: 'lane-partner', title: 'مراجعة الشركاء', description: 'القبول أو الرفض الأولي يبقى واضحًا داخل نفس لوحة العمل.' },
  { id: 'lane-marketing', title: 'مراجعة التسويق', description: 'بعد القبول الأولي تنتقل البطاقة لتثبيت العرض التسويقي.' },
  { id: 'lane-catalog', title: 'نشر الكتالوج', description: 'بعد الاعتماد النهائي تصبح البطاقة عامة لكل الشركاء.' },
] as const;
...
export const dshPromotionCandidates: ReadonlyArray<DshPromotionCandidate> = [
  { id: 'product-burger', kind: 'product', title: 'برغر كلاسيك', subtitle: 'منتج عالي الطلب مناسب لعروض الرفع السريع.', availability: 'متاح وبمخزون جيد', eligibility: 'eligible', status: 'draft', offerHint: 'اقترح خصمًا قصيرًا أو باقة مزدوجة.' },
  { id: 'store-yasmin', kind: 'store', title: 'متجر الياسمين', subtitle: 'فرع جاهز للظهور الترويجي مع نشاط ثابت.', availability: 'جاهز للظهور', eligibility: 'review', status: 'partner-review', offerHint: 'اربط العرض بوقت الذروة أو حزمة توصيل.' },
  { id: 'product-dessert', kind: 'product', title: 'حلويات موسمية', subtitle: 'منتج يطلب مراجعة قبل الترويج الواسع.', availability: 'بحاجة لمراجعة', eligibility: 'blocked', status: 'marketing-rejected', offerHint: 'أعد ضبط التوفر أو أضف سبب الرفض.' },
  { id: 'store-olaya', kind: 'store', title: 'فرع العليا', subtitle: 'أداء ممتاز ويستحق الإبراز في الرئيسية.', availability: 'جاهز تماماً', eligibility: 'eligible', status: 'marketing-ready', offerHint: 'توصيل مجاني أو خصم 20%.' },
];
...
export function transitionApprovalStage(current: ApprovalStage, action: 'approve' | 'reject' | 'fix'): ApprovalStage {
  ...
  switch (current) {
    ...
    case 'field-submitted':    return 'partner-review';
    case 'partner-review':     return 'partner-approved';
    case 'partner-approved':   return 'marketing-review';
    case 'marketing-review':   return 'marketing-approved';
    case 'marketing-approved': return 'catalog-adopted';
    case 'catalog-adopted':    return 'client-visible';
...
export function resolveNextOwner(stage: ApprovalStage): ApprovalSourceSurface {
  switch (stage) {
    ...
    case 'partner-review':     return 'control-panel-partners';
    case 'partner-approved':
    case 'marketing-review':   return 'control-panel-marketing';
    case 'marketing-approved':
    case 'catalog-adopted':    return 'control-panel-catalog';
    case 'client-visible':     return 'app-client';
    case 'rejected':
    case 'needs-fix':          return 'app-partner';
    default:                   return 'control-panel-partners';
  }
}

export type ClientVisibilityOptions = { mediaPolicy?: string };
...
export function canRenderInClientSurface(stage: string | undefined, entityType?: ApprovalEntityType, options?: ClientVisibilityOptions): boolean {
  ...
  if (entityType === 'product-media') {
    ...
    if (isAllowedPolicy && isAllowedStage) return true;
  }
  if (stage === 'catalog-adopted' || stage === 'marketing-review' || stage === 'marketing-approved' || stage === 'partner-approved' || stage === 'partner-review' || stage === 'partner-submitted' || stage === 'field-submitted' || stage === 'needs-fix' || stage === 'rejected') return false;
  return false;
}
...
let _globalStore: ApprovalRecord[] = [
  ...
  { id: 'price-conflict-1', entityType: 'product', source: 'app-partner', stage: 'partner-submitted', title: 'تعارض سعر: تفاح رويال غالا (سعر الشريك 32 ر.س مقابل المرجعي 18 ر.س)', submittedAt: new Date(Date.now() - 3600_000 * 12).toISOString(), metadata: { systemNote: 'فرق السعر يتجاوز حد التسامح المسموح به (20%).' }, auditTrail: [] },
  { id: 'price-conflict-2', entityType: 'product', source: 'app-partner', stage: 'partner-review', title: 'تعارض سعر: حليب عضوي 1.5 لتر (سعر الشريك 25 ر.س مقابل المرجعي 11 ر.س)', submittedAt: new Date(Date.now() - 3600_000 * 6).toISOString(), metadata: { systemNote: 'تعارض تسعير نشط مع الفئات المجاورة.' }, auditTrail: [] },
  { id: 'barcode-conflict-1', entityType: 'product', source: 'app-partner', stage: 'partner-submitted', title: 'تعارض باركود: خبز قمح كامل (الباركود 6281100223344 مستخدم بالفعل لـ منتج آخر)', submittedAt: new Date(Date.now() - 3600_000 * 8).toISOString(), metadata: { systemNote: 'الباركود متطابق مع منتج نشط في الكتالوج.' }, auditTrail: [] },
];

// ── Selectors ────────────────────────────────────────────────────────
export function getAllApprovalRecords(): ApprovalRecord[] { return _globalStore; }
export function getPartnerQueueRecords(): ApprovalRecord[] { return _globalStore.filter(r => (PARTNER_QUEUE_STAGES as readonly any[]).indexOf(r.stage) >= 0); }
export function getPartnerIntakeItems(): ApprovalRecord[] { return getPartnerQueueRecords(); }
export function getMarketingQueueRecords(): ApprovalRecord[] { return _globalStore.filter(r => (MARKETING_QUEUE_STAGES as readonly any[]).indexOf(r.stage) >= 0); }
export function getCatalogQueueRecords(): ApprovalRecord[] { return _globalStore.filter(r => (CATALOG_QUEUE_STAGES as readonly any[]).indexOf(r.stage) >= 0); }
export function getClientVisibleRecords(): ApprovalRecord[] { return _globalStore.filter(r => r.stage === 'client-visible'); }

// ── Mutations ────────────────────────────────────────────────────────
export function upsertApprovalRecord(record: Partial<ApprovalRecord> & { id: string }): void {
  let idx = -1;
  for (let i = 0; i < _globalStore.length; i++) { if (_globalStore[i].id === record.id) { idx = i; break; } }
  if (idx >= 0) { _globalStore = _globalStore.map(r => (r.id === record.id ? { ...r, ...record } : r)); }
  else { _globalStore = [..._globalStore, record as ApprovalRecord]; }
}

export function moveApprovalRecordToStage(id: string, toStage: ApprovalStage, owner: ApprovalSourceSurface, actionLabel: string): void {
  _globalStore = _globalStore.map(r => {
    if (r.id !== id) return r;
    const entry: AuditTrailEntry = { at: new Date().toISOString(), fromStage: r.stage, toStage, owner, actionLabel };
    return { ...r, stage: toStage, auditTrail: [...(r.auditTrail || []), entry] };
  });
}

// =====================================================================
// DSH Shared Partner Activation, Documents & Overrides SSoT
// =====================================================================
...
export type DshPartnerDocumentVerification = {
  id: string; kind: DshPartnerDocumentKind; label: string;
  status: 'uploaded' | 'missing' | 'verified' | 'rejected';
  uploadedAt?: string; rejectionReason?: string;
  verifiedByFieldAgent?: string; fieldVisitDate?: string;
  fieldEvidencePhoto?: string; geoCoordinates?: string;
};

export type DshPartnerCatalogOverride = {
  productId: string; priceOverride?: string; stockOverride?: number;
  availableOverride?: boolean; prepNoteOverride?: string;
  owner: 'partner' | 'cp_manager'; state: 'active' | 'draft' | 'pending_review'; lastUpdated: string;
};

let _globalPartnerStatuses: Record<string, DshPartnerActivationStatus> = {
  'partner-saha': 'client_visible', 'partner-shorouq': 'submitted',
  'partner-zawya': 'documents_uploaded', 'partner-nokhba': 'ops_approved',
};
...
let _globalPartnerOverrides: Record<string, DshPartnerCatalogOverride[]> = {
  ...
  'partner-shorouq': [{ productId: 'item-chicken-2', priceOverride: '18.00 ر.س', stockOverride: 120, availableOverride: true, prepNoteOverride: 'تحضير طازج يستغرق ١٥ دقيقة', owner: 'partner', state: 'active', lastUpdated: 'اليوم 09:20' }],
  'partner-zawya': [{ productId: 'item-bread-1', priceOverride: '12.00 ر.س', stockOverride: 0, availableOverride: false, owner: 'partner', state: 'pending_review', lastUpdated: 'أمس 12:00' }],
  'partner-nokhba': [{ productId: 'canonical-product-field-lead-5-featured', priceOverride: '95.00 ر.س', stockOverride: 300, availableOverride: true, prepNoteOverride: 'تغليف ملكي خاص للهدايا الميدانية', owner: 'cp_manager', state: 'active', lastUpdated: 'اليوم 12:15' }],
};
...
export function updatePartnerDocumentStatus(partnerId: string, docId: string, status: 'uploaded' | 'missing' | 'verified' | 'rejected', reason?: string): void {
...
export function getPartnerComplaints(): PartnerComplaint[] { return _globalPartnerComplaints; }
...
export function updatePartnerComplaintStatus(id: string, status: 'open' | 'investigating' | 'resolved', notes?: string): void {
  _globalPartnerComplaints = _globalPartnerComplaints.map(c => {
    if (c.id !== id) return c;
    return { ...c, status, description: notes ? `${c.description}\n[ملاحظة التدقيق]: ${notes}` : c.description };
  });
}

export function getPartnerModifications(): PartnerModificationRequest[] { return _globalPartnerModifications; }
  ...
  _globalPartnerModifications = _globalPartnerModifications.map(r => {
    ...
    if (status === 'approved') {
      ...
      if (partner) {
        ...
        if (storeNameChange) { (partner as any).storeName = storeNameChange.new; }
      }
    }
    return { ...r, status };
  });
...
export function getPartnerDisputes(): PartnerDispute[] { return _globalPartnerDisputes; }
...
export function updatePartnerDisputeStatus(id: string, status: string, notes?: string): void {
  _globalPartnerDisputes = _globalPartnerDisputes.map(d => {
    if (d.id !== id) return d;
    return { ...d, status, ...(notes ? { type: `${d.type} (ملاحظة: ${notes})` } : {}) };
  });
}

export function getPromotionCandidates(): DshPromotionCandidate[] { return _globalPromotionCandidates; }
...
export function updatePromotionCandidateStatus(id: string, status: DshPromotionIntentStatus, eligibility?: 'eligible' | 'review' | 'blocked'): void {
  _globalPromotionCandidates = _globalPromotionCandidates.map(c => {
    if (c.id !== id) return c;
    return { ...c, status, ...(eligibility ? { eligibility } : {}) };
  });
}
