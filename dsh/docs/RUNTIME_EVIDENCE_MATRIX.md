...
|---|---|---|---|---|---|---|---|---|
| `DSH-RUN-P014-01` | app-client discovery + storefront | `HomeScreen.tsx`; `SearchScreen.tsx`; `StoreScreen.tsx`; `DshClientSurface.tsx`; typed client bridge; `dsh-discovery-stores-transport.ts`; `dsh-discovery-stores-runtime-config.ts` | `preview-fallback`, `props-driven`, `local-state`, `typed-client-boundary`, `postgres-api-proven`, `frontend-transport-bound` | `BATCH_9C_FRONTEND_RUNTIME_TRANSPORT_READY_FOR_E2E_PROOF` | `FRONTEND_TRANSPORT_BOUND__E2E_PROOF_PENDING` | `api-db-runtime-proven__transport-bound__screen-runtime-unproven` | Transport is wired: `DshClientSurface` calls `GET /stores` when `EXPO_PUBLIC_DSH_API_BASE_URL` is set; real response flows through bridge to screen. E2E proof (start backend + hit endpoint + confirm `openapi-response` source in bridge + screen shows live data) has not been captured yet. | Batch 9D E2E proof: start Go backend with PostgreSQL, set env var, confirm request/response/screen evidence |
