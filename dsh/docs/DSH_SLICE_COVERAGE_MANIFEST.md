...
| J-003 | Checkout / Payment | Business | client, WLT, partner, operator | finance + operations | full WLT ownership of payment/money | IMPLEMENTATION_STARTED — auth.openapi.yaml + wlt.openapi.yaml + dsh.openapi.yaml contracts designed; Go backend handlers 003A/003B/003C/003E implemented; 003A/003B production BearerAuth backend path exists and app-client checkout transport can send Bearer token; DEV fallback remains X-Client-Id; runtime auth-service proof, WLT payment runtime/security proof, and visual proof pending; 003C callback-primary flow documented; 003D/003E blocked on 003C WLT runtime proof |
...
| `DSH-SLICE-003A` | J-003 | Client reviews cart and confirms serviceability lock | client | app-client (cart) | shared serviceability model | app-captain (N/A: not yet assigned); WLT (N/A: payment not started); app-partner (N/A: J-001); app-field (N/A: J-006); control-panel (N/A: no CP action at cart step) | view cart; remove item; proceed to checkout | loading, empty, error, blocked, retry, serviceable, not_serviceable | preview/local-state | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_AUTH_CLIENT_BOUND_RUNTIME_PENDING (dsh.openapi.yaml GET /cart/serviceability + backend checkout_handler.go implemented; backend BearerAuth production path + app-client Bearer transport implemented; DEV X-Client-Id fallback remains) | WLT starts at payment intent (003C) | client auth — live auth-service BearerAuth proof pending; X-Client-Id is DEV fallback only | yes — 7 states required | NEEDS_RUNTIME_EVIDENCE — blocked on live auth-service proof + CartScreen runtime/visual proof | DSH_AUTH_MODE=production live auth service proof + CartScreen live API runtime proof + visual proof (all 7 states) | BLOCKED_WITH_REASON — live auth-service proof + live CartScreen/runtime/visual proof pending; slice file fully upgraded and truth-synced (DSH_JOURNEY_003_AUTH_CLIENT_BINDING_EXECUTION-20260604) | Prove DSH_AUTH_MODE=production against auth.openapi.yaml GET /auth/session; capture CartScreen runtime + visual proof; do NOT mark PASS until runtime + visual proof |
| `DSH-SLICE-003B` | J-003 | Client submits checkout intent; system reserves items and prepares order session | client | app-client / DshCheckoutIntentScreen | DSH backend checkout session service | app-captain (N/A); app-partner (N/A: intake is 003D); app-field (N/A: J-006); control-panel (N/A: no operator action) | confirm checkout; cancel; change address | loading, address_entry, intent_created, intent_failed, blocked, error | preview/local-state | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_AUTH_CLIENT_BOUND_RUNTIME_PENDING (dsh.openapi.yaml POST /checkout/intent + backend checkout_handler.go implemented; backend BearerAuth production path + app-client Bearer transport implemented; DEV X-Client-Id fallback remains; 003A runtime proof required) | WLT receives session token at 003C | client auth (upstream from 003A) — live auth-service BearerAuth proof pending | yes — 6 states required | NEEDS_RUNTIME_EVIDENCE — blocked on 003A runtime proof + live auth-service proof + screen proof | 003A runtime proof + DshCheckoutIntentScreen runtime proof + WLT/session-token flow proof + visual proof | BLOCKED_WITH_REASON — 003A runtime proof + live auth-service proof + live screen proof required; slice truth-synced (DSH_JOURNEY_003_AUTH_CLIENT_BINDING_EXECUTION-20260604) | Await/prove 003A; capture DshCheckoutIntentScreen runtime + visual proof; do NOT mark 003C PASS until 003B runtime is proven |
| `DSH-SLICE-003C` | J-003 | Payment executed via WLT; DSH receives payment-confirmed callback and stores reference ID only | WLT (primary); DSH backend (callback) | WLT (external boundary); app-client (WltBoundaryBanner) | DSH backend callback handler; control-panel finance (read-only) | app-captain (N/A); app-partner (N/A: intake is 003D); app-field (N/A: J-006); WLT wallet mutation (excluded: WLT owns entirely) | Pay via WLT; (auto) receive callback; retry on WLT failure | awaiting_wlt_confirmation, payment_confirmed, payment_failed, loading, error | WLT-owned (payment); DSH stores reference ID only | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_AUTH_HEADERS_ENFORCED (dsh.openapi.yaml POST /checkout/payment-callback + backend checkout_handler.go implemented) — requires X-WLT-Callback-Token, X-WLT-Event-Id, and Idempotency-Key; dsh_checkout_intents.wlt_callback_event_id persists replay state; final WLT runtime/security/HMAC proof and 003B runtime proof remain blocking | Full WLT ownership — DSH callback only; no finance mutation; requested_amount_minor_units in DB is non-authoritative snapshot only | WLT-managed auth — shared-token validation implemented; final HMAC/signature semantics pending WLT security spec | yes — WltBoundaryBanner states required | NEEDS_WLT_RUNTIME_PROOF | WLT runtime/security proof + final callback semantics + 003B runtime proof + WLT E2E proven + DSH callback runtime evidence | BLOCKED_WITH_REASON — WLT runtime/security proof + 003B runtime proof + visual proof required; slice fully upgraded (DSH_SLICE_003B_003E_BLOCKED_COMPLIANCE_CLOSURE-20260604-174700) | WLT team publishes runtime/security proof; prove DSH callback receive/store path with WLT confirmed/failed events; do NOT start 003D or 003E PASS closure until 003C is proven |
| `DSH-SLICE-003D` | J-003 | DSH creates order record after WLT payment confirmation is stored; hands off to partner + ops monitor | DSH backend / order service; app-partner; control-panel ops | DSH backend / order service | app-partner (intake); control-panel ops monitor | app-captain (N/A: assigned in J-005); app-field (N/A: J-006); WLT (excluded: already provided payment confirmation) | (auto) create order; view order (client); view new order (partner) | payment_confirmed, order_CREATED, order_creation_failed, notification_sent, loading, error | DSH backend domain (order record); WLT payment reference (read-only) | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_POSTGRES_REQUIRED — POST /orders and GET /orders/{id} exist in dsh.openapi.yaml and backend implementation; order creation is a separate post-callback step, not inside POST /checkout/payment-callback; requires 003B + 003C runtime proof before PASS | DSH reads WLT payment reference ID only; no DSH finance mutation | DSH internal + partner + operator auth | yes — client confirmation + partner intake + CP ops monitor | NEEDS_RUNTIME_EVIDENCE | 003B + 003C runtime proof + Postgres migration applied + partner notification proven + runtime proof + visual proof | BLOCKED_WITH_REASON — depends on 003B + 003C runtime proof; slice fully upgraded (DSH_SLICE_003D_FULL_UNIVERSAL_CLOSURE-20260604-181500) | Await 003B + 003C runtime proof; prove separate post-callback order creation handoff; do NOT start J-004 partner lifecycle until 003D PASS |
| `DSH-SLICE-003E` | J-003 | Client receives failure feedback + recovery path when WLT payment fails; cart preserved | client | app-client / CheckoutFailureScreen | DSH backend (failure callback); app-client cart (preserved) | app-captain (N/A); app-partner (N/A); app-field (N/A: J-006); control-panel (NOT_APPLICABLE at this step — escalation in J-004) | retry payment; cancel checkout; contact support | payment_failed, retry_in_progress, cancelled, cart_preserved, loading, error | WLT-owned (failure reason); DSH stores failure event | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_SCREEN_REGISTERED — DELETE /checkout/intent/{id} and failure callback path are implemented; DshCheckoutFailureScreen is registered; blocked by 003C WLT runtime proof, WLT failure error spec, live screen wiring proof, and visual proof | WLT owns failure reason; DSH displays only; no finance mutation | client auth (session from 003B) | yes — 6 states + cart preserved state | NEEDS_RUNTIME_EVIDENCE | 003C runtime proof + WLT error spec + CheckoutFailureScreen runtime + visual proof | BLOCKED_WITH_REASON — 003C runtime proof + WLT error spec + screen runtime/visual evidence required; slice fully upgraded (DSH_SLICE_003E_FULL_UNIVERSAL_CLOSURE-20260604-182000) | Await 003C runtime proof + WLT error spec; wire/prove CheckoutFailureScreen retry + cancel CTAs; J-003 NOT closed until 003E PASS |
| `DSH-SLICE-004A` | J-004 | Client views order tracking timeline | client | app-client (orders) | partner (lifecycle events), captain (milestones), control-panel | WLT (N/A: read-only order events) | view timeline; open details; contact support | loading, error, success, offline, cancelled | preview/local-state | CONTRACT_DESIGNED_BACKEND_IMPLEMENTED_CLIENT_BOUND (dsh.openapi.yaml GET /orders/{id} + backend implementation + client ts exist; runtime proof blocked by J-003) | WLT owns refund only | client auth | yes | NEEDS_RUNTIME_EVIDENCE | Order event API + timeline screen proven | DEFERRED_WITH_REASON — depends on J-003 closure; slice fully upgraded (DSH_SLICE_004A_CLIENT_ORDER_TRACKING_FINAL_CLOSURE-20260605-001200) | Await J-003 full closure |
| `DSH-SLICE-004B` | J-004 | Partner accepts/rejects/prepares/marks ready | partner | app-partner (orders) | control-panel (ops visibility), app-client (order status) | app-captain (N/A: not yet assigned) | accept; reject; prepare; mark ready | loading, empty, error, success, offline, blocked, retry | preview/local-state | NOT_READY_FOR_API | WLT only if reversal becomes financial | partner auth | yes | NEEDS_RUNTIME_EVIDENCE | Partner acceptance + handoff API proven | DEFERRED_WITH_REASON — depends on 003D; slice fully upgraded (DSH_SLICE_004B_FULL_UNIVERSAL_CLOSURE-20260604-183400) | Close 003D first |
| `DSH-SLICE-004C` | J-004 | Support escalation link: client raises support ticket | client, operator | app-client + control-panel (support) | partner (context owner) | app-captain (N/A: support not delivery) | raise issue; attach evidence; escalate | loading, success, error | preview/local-state | NOT_READY_FOR_API | WLT only if financial outcome | client + operator auth | yes | NEEDS_RUNTIME_EVIDENCE | Support ticket API + escalation flow proven | DEFERRED_WITH_REASON — depends on J-003/J-004 runtime; slice fully upgraded (DSH_SLICE_004C_SUPPORT_ESCALATION_FINAL_CLOSURE-20260604-184000) | Await J-003/J-004 close; then design support escalation API |
| `DSH-SLICE-004D` | J-004 | Cancellation request: client cancels an active order | client, partner | app-client + app-partner | control-panel (audit) | app-captain (N/A: delivery not started) | request cancellation; confirm cancellation; reject cancellation | loading, success, error, blocked | preview/local-state | NOT_READY_FOR_API | WLT only if refund is triggered | client + partner auth | yes | NEEDS_RUNTIME_EVIDENCE | Cancellation API + cross-surface state proven | DEFERRED_WITH_REASON — depends on 003D/004B; slice fully upgraded (DSH_SLICE_004D_CANCELLATION_FINAL_CLOSURE-20260604-185000) | Await 003D + 004B close; then design cancellation API |
