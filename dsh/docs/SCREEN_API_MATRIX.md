...
| `DSH-SAPI-P014-11` | `control-panel` | `/finance` | `FinanceHubScreen.tsx`; `FinanceHubScreens.tsx`; `WltBoundaryBanner.tsx` | inspect read-only finance visibility | `loading`, `error`, `success`, `blocked` | `BLOCKED_WITH_REASON` | WLT owns settlement, payout, refund, commission, and ledger semantics | finance remains a read-only bridge, not a DSH API surface | keep blocked by WLT |
