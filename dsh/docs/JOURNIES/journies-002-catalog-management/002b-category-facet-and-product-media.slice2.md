<!--
Canonical human-review path: dsh/docs/journies-002-catalog-management/002b-category-facet-and-product-media.slice2.md
Original generated path: dsh/docs/slices/human-sequential-closure-v3/slices/J-002-deep-catalog-management/DSH-SLICE-002B-category-facet-and-product-media.md
...
- Journey folder: `dsh/docs/journies-002-catalog-management/`
- Slice file: `dsh/docs/journies-002-catalog-management/002b-category-facet-and-product-media.slice2.md`
- Slice order inside journey: `2` of `6`
...
# DSH-SLICE-002B — Category Facet and Product Media
...
| Parent Journey | `J-002 — Deep Catalog Management` |
| Journey Type | `business` |
| Business Outcome | assign categories/facets/mediaKey without local media drift |
| Primary Actor | `partner` |
| Primary Surface / Area | `app-partner + central media/data` |
| Control Panel Owner | `catalog governance` |
| WLT Boundary | `none` |
...
1. يبدأ الممثل الأساسي `partner` من السطح `app-partner + central media/data`.
2. الهدف العملي: assign categories/facets/mediaKey without local media drift.
...
|---|---|---|---|
| `app-partner + central media/data` | primary | ينفذ الهدف الأساسي للشريحة | screenshot/log/test |
...
- انقل/وحّد demo/mock/preview data إلى `dsh/frontend/data` فقط.
- انقل/وحّد media fixtures إلى `dsh/frontend/media-fixtures` فقط.
- استبدل النسخ المحلية بـ IDs/mediaKey/references.
...
|---|---|---|---|---|---|
| GAP-002B-01 | CategoryManagementScreen.tsx exists. ProductMediaScreen.tsx exists. OpenAPI: createCategory/listCategories/getCategory/updateCategory/deleteCategory + media upload-intent/complete/delete all defined. Backend: categories_handler.go + media_handler.go + media_runtime_handler.go exist. No local media drift — mediaKey pattern used in InventoryCatalogScreen. | CODE_EXISTS_VERIFIED | app-partner | None — visual screenshot evidence required for PASS | NEEDS_VISUAL_EVIDENCE |
| GAP-002B-02 | CODE_VERIFICATION_RUN 2026-06-12: tsc --noEmit = PASS (0 errors). go test ./... = PASS. categories_handler.go + ProductMediaScreen.tsx + listProductMediaByProductIDs batch query confirmed compile-clean. Evidence: tools/registry/runs/J-002-20260612-182200/tsc-noemit.txt + go-test.txt | CODE_VERIFIED_TSC_GOTEST | app-partner + backend | None — code gate passed | NEEDS_VISUAL_EVIDENCE |
...
- Parent Journey: J-002 — Deep Catalog Management
- Slice: DSH-SLICE-002B — Category Facet and Product Media
- Primary actor: partner
- Primary surface/area: app-partner + central media/data
- Business outcome: assign categories/facets/mediaKey without local media drift
- SESSION_ID: DSH-SLICE-002B_FINAL_CLOSURE-YYYYMMDD-HHMMSS
...
- evidence folder: tools/registry/runs/DSH-SLICE-002B_FINAL_CLOSURE-YYYYMMDD-HHMMSS/
- evidence zip: tools/registry/runs/DSH-SLICE-002B_FINAL_CLOSURE-YYYYMMDD-HHMMSS/DSH-SLICE-002B_FINAL_CLOSURE-YYYYMMDD-HHMMSS.zip
