...
يجب تحليل ومراجعة وتعديل وإضافة وتصحيح كل ما يلزم لإغلاق الشريحة فعليًا من كل الطبقات، بشرط أن يكون داخل نطاق الشريحة أو مرتبطًا بها مباشرة.
...
- هذا الأمر لا يحصر الإغلاق في `docs`.
- يسمح بتعديل `dsh.openapi.yaml`، `dsh/backend/**`، `dsh/frontend/**` وكل ما يلزم داخل نطاق الشريحة.
- يمنع توسيع النطاق عشوائيًا.
- يفرض evidence folder وZIP باسم `[SESSION_ID].zip`.
- يفرض تحديث ملفات الحقيقة حتى لا يبقى تناقض بين manifest/matrices/slice file.
- يفرض on-demand retrieval وعدم تضخيم البيانات.
- يفرض حدود WLT وعدم إدخال money semantics داخل DSH.
- يفرض عدم ادعاء PASS إذا الأدلة ناقصة.
