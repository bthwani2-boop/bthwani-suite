        ...
        <Box style={{ flex: 1, minWidth: 0, gap: 2, alignItems: direction === 'rtl' ? 'flex-end' : 'flex-start' }}>
          ...
          {product.publishStage ? (
            <StatusTimeline
              ...
              metadata={getPartnerQueueRecords().find((record) => record.id === product.id || record.title.includes(product.name))?.metadata}
            />
          ) : null}
...
function InventoryCatalogContent({ storeName, branchLabel, activeZoneLabel, todayHoursLabel, canonicalStoreId }: InventoryCatalogContentProps) {
  const { direction } = useDirection();
