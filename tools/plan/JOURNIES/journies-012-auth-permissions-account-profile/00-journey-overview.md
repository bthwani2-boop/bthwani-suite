...
New canonical human-review folder: dsh/docs/journies-012-auth-permissions-account-profile/
...
# J-012 — Auth, Permissions, Account/Profile Dependency
...
|---|---|
| Journey ID | `J-012` |
| Name | Auth, Permissions, Account/Profile Dependency |
| Type | `foundation` |
| Primary Actors | client, partner, captain, field, operator |
| Control Panel / Domain Owner | auth/service boundary |
| WLT Boundary | auth only for financial authorization |
| Planning Status | `REQUIRED_FOR_ALL_BLOCKED_RUNTIME_SLICES` |
| Business Outcome | Every surface has clear login/session/role/permission behavior and APIs enforce Bearer/role access before runtime closure. |
...
|---|---|---|---|---|---|---|
| `DSH-SLICE-012A` | Auth OpenAPI and Runtime Baseline | developer | auth.openapi.yaml + auth-service | contract and runtime align | `slices/j-012-auth-permissions-account-profile-dependency/DSH-SLICE-012A-auth-openapi-and-runtime-baseline.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-012B` | Client Session and Profile Binding | client | app-client + backend | client identity binds cart/order/profile | `slices/j-012-auth-permissions-account-profile-dependency/DSH-SLICE-012B-client-session-and-profile-binding.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-012C` | Partner Captain Field Role Boundaries | partner/captain/field | mobile apps + backend | role access prevents cross-surface leakage | `slices/j-012-auth-permissions-account-profile-dependency/DSH-SLICE-012C-partner-captain-field-role-boundaries.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-012D` | Control Panel Operator RBAC | operator | control-panel + backend | operator roles gate actions/screens | `slices/j-012-auth-permissions-account-profile-dependency/DSH-SLICE-012D-control-panel-operator-rbac.md` | NOT_CLOSED_BY_THIS_FILE |
| `DSH-SLICE-012E` | Expired Token Offline Retry UX | all actors | all surfaces | session expiry/offline/retry states are visible and safe | `slices/j-012-auth-permissions-account-profile-dependency/DSH-SLICE-012E-expired-token-offline-retry-ux.md` | NOT_CLOSED_BY_THIS_FILE |
