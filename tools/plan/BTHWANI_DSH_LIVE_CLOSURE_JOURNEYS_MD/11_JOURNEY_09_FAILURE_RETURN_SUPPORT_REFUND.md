...
# 11 — الرحلة 09: فشل التوصيل والإرجاع والدعم والاسترداد

## الهدف

إغلاق الحالات غير السعيدة: فشل التوصيل، return to store، escalation، refund عبر WLT.

## النطاق

app-client + app-partner + app-captain + control-panel + DSH backend + WLT backend.
...
### Slice 09.1 Failed Delivery Without Return

**هدف الشريحة:** فشل نهائي بدون إرجاع.

#### التجريب الحي المطلوب

- fail-delivery return_required=false
- FAILED_DELIVERY
- CP exception
- client notice
...
- افحص/ارفض: refund executed in DSH.
- افحص/ارفض: reason optional.
- افحص/ارفض: status غير متناسق.
...
### Slice 09.2 Failed Delivery With Return

**هدف الشريحة:** فشل مع إرجاع للمتجر.
...
- افحص/ارفض: confirm return من status خاطئ.
- افحص/ارفض: partner لا يرى return.
- افحص/ارفض: لا proof.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
...
### Slice 09.3 Support Escalation

**هدف الشريحة:** فتح مشكلة من كل actor.
...
- افحص/ارفض: ticket بلا order_id.
- افحص/ارفض: actor spoofing.
- افحص/ارفض: no SLA.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
...
### Slice 09.4 Refund Creation in WLT

**هدف الشريحة:** إنشاء refund مالي في WLT.
...
- افحص/ارفض: DSH يغير wallet.
- افحص/ارفض: refund بلا idempotency.
- افحص/ارفض: no ledger.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
...
### Slice 09.5 Refund Callback to DSH

**هدف الشريحة:** WLT يبلغ DSH بالاسترداد.

#### التجريب الحي المطلوب

- POST /orders/{id}/refund-callback
- bad token
- success/fail
...
- افحص/ارفض: callback بلا token.
- افحص/ارفض: DSH يحسب amount.
- افحص/ارفض: client لا يرى status.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
- أضف callback handling + UI refresh.
...
### Slice 09.6 UX for Exceptions

**هدف الشريحة:** مراجعة الرسائل والتدفقات.

#### التجريب الحي المطلوب

- screenshots failure/return/refund/support
...
- افحص/ارفض: لغة اتهامية.
- افحص/ارفض: CTA مربك.
- افحص/ارفض: لا next step.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
### Slice 09.7 Cleanup of Exception Logic

**هدف الشريحة:** فحص support/refund/failure code.

#### التجريب الحي المطلوب

- rg refund fail support
- depcruise
- jscpd
...
- افحص/ارفض: refund duplicated in DSH/WLT.
- افحص/ارفض: support screens مكررة.
- افحص/ارفض: dead exception queue.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
