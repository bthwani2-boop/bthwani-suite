...
|---|---|---|---|---|---|---|---|---|
| `DSH-RUN-P014-01` | app-client discovery + storefront | `HomeScreen.tsx`; `SearchScreen.tsx`; `StoreScreen.tsx`; `DshClientSurface.tsx`; typed client bridge; `dsh-discovery-stores-transport.ts`; `dsh-discovery-stores-runtime-config.ts` | `runtime-proven` | `DSH_SLICE001_SCREEN_RUNTIME_PROVEN` | app-client / GET /stores E2E runtime screenshot captured | `api-db-runtime-proven__transport-bound__screen-runtime-proven` | none — partner + CP screen proof captured in `tools/registry/runs/DSH_SLICE001_FINAL_SCREEN_RUNTIME-20260603-194700/`; all 3 visibility gates verified on device and browser against live Postgres | none — DSH-SLICE-001 cross-surface runtime proven; transition to DSH-SLICE-002 |
