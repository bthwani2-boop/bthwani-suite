...
# 12 — الرحلة 10: المالية والتسويات والمحافظ وLedger

## الهدف

إغلاق finance bridge بعد التسليم: settlements، wallets، ledger، read-only finance في DSH/CP.

## النطاق

WLT backend + WLT frontend/dsh shared + control-panel finance + app-client/captain/partner finance surfaces.
...
### Slice 10.1 Settlement Candidate After Delivery

**هدف الشريحة:** تحويل DELIVERED إلى candidate للتسوية.
...
- افحص/ارفض: DSH يحسب payout.
- افحص/ارفض: candidate من order غير delivered.
- افحص/ارفض: amount غير موثق.
...
### Slice 10.2 Settlement Lifecycle

**هدف الشريحة:** PENDING → PROCESSING → COMPLETED/FAILED.
...
- افحص/ارفض: لا ledger entries.
- افحص/ارفض: status bypass.
- افحص/ارفض: no idempotency.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
### Slice 10.3 Wallet Summary

**هدف الشريحة:** عرض wallet summary حسب actor.
...
- افحص/ارفض: actor sees others wallet.
- افحص/ارفض: summary mock.
- افحص/ارفض: negative balance غير مفسر.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
### Slice 10.4 Wallet Transactions

**هدف الشريحة:** عرض ledger entries.
...
- افحص/ارفض: entries بلا reference.
- افحص/ارفض: sorting خاطئ.
- افحص/ارفض: local-only rows.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
### Slice 10.5 Control Panel Finance Hub

**هدف الشريحة:** مراجعة FinanceHub والبيانات.
...
- افحص/ارفض: preview finance component.
- افحص/ارفض: mutating action في DSH CP.
- افحص/ارفض: لا read-only banner.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
- استبدل preview بـ runtime أو dev-only.
...
### Slice 10.6 Financial Design Review

**هدف الشريحة:** تجربة الشاشات المالية.
...
- افحص/ارفض: مبالغ غير واضحة.
- افحص/ارفض: CTA خطر بلا confirm.
- افحص/ارفض: لون أحمر/أخضر بلا معنى.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
...
- أضف maker-checker/confirm عند المال.
...
### Slice 10.7 Finance Boundary Cleanup

**هدف الشريحة:** فحص WLT/DSH shared.

#### التجريب الحي المطلوب

- rg walletPreview mock fallback amount ledger
- guard:wlt-dsh-shared-ownership
...
- افحص/ارفض: surface-specific hook داخل WLT shared.
- افحص/ارفض: mock data.
- افحص/ارفض: financial mutation in DSH.
- صنف كل ملف متأثر: `KEEP_ACTIVE / REFACTOR_SPLIT / MERGE_DUPLICATE / RETIRE_DEAD / MOVE_TO_OWNER / FIX_REQUIRED / BLOCKED_NEEDS_EVIDENCE`.
- لا تحذف أو تنقل قبل فحص imports/exports/routes/navigation/runtime/tests.
- أي preview/demo/mock/fallback في runtime يجب عزله أو حذفه أو ربطه بمصدر حقيقي.
- أي تكرار UI يجب أن يعود إلى `@bthwani/ui-kit` أو مكون service-owned واضح.
