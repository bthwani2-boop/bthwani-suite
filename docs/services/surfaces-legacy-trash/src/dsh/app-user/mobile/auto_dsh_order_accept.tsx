// Auto-generated screen for dsh_order_accept
// Surface: app-client | Service: dsh | Operation: POST /api/dsh/orders/:orderId/accept
...
export const auto_dsh_order_accept: React.FC<Props> = ({ onNavigate, navigation, route }) => {
  ...
  const submit = useCallback(async () => {
    ...
    setState('loading');
    try {
      const url = `${getBaseUrl()}/api/dsh/orders/${encodeURIComponent(id)}/accept`;
  ...
  if (state === 'loading') {
    return (
      <ScreenWrapper
        state="loading"
        loadingMessage={t('dsh.app-client.mobile.auto_dsh_order_accept.loadingMessage')}
        screenName="auto_dsh_order_accept"
        operationName="dsh_order_accept"
      />
    );
  }
  ...
  if (state === 'error') {
    return (
      <ScreenWrapper
        state="error"
        errorMessage={t('dsh.app-client.mobile.auto_dsh_order_accept.errorMessage')}
  ...
  return (
    <ScreenWrapper state="content">
      <View style={styles.container}>
        <Text style={[styles.title, textAlignStart]}>قبول طلب</Text>
        <Text style={[styles.subtitle, textAlignStart]}>معرف الطلب (orderId)</Text>
        <TextInput
          style={[styles.input, textAlignStart]}
          value={orderId}
          onChangeText={setOrderId}
          placeholder={t('dsh.app-client.mobile.auto_dsh_order_accept.orderIdPlaceholder')}
        ...
        {success && (
          <View style={styles.result}>
            <Text style={[styles.resultText, textAlignStart]}>{t('dsh.app-client.mobile.auto_dsh_order_accept.resultSuccess')}</Text>
          </View>
        )}
...
export default auto_dsh_order_accept;
