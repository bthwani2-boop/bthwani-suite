...
|---|---|---|---|---|---|---|---|---|---|
| `DSH-SAPI-P014-01` | `app-client` | `/app-client/discovery` | `HomeScreen.tsx`; `SearchScreen.tsx`; `StoreScreen.tsx` | open a destination, store, or category | `loading`, `empty`, `error`, `success`, `offline` | `L7_CLOSED` (app-client only) | no WLT boundary yet | pending cross-surface proof for Slice 001 | `NEEDS_HUMAN_EVIDENCE_SLICE_001` |
...
Exit gate:
`READY_FOR_ONE_OPENAPI_ENDPOINT` is passed for `DSH-SAPI-P014-01` after visual review signoff and definition of the single discovery endpoint. Subsequent rows remain blocked by WLT/Auth or contract gates.
