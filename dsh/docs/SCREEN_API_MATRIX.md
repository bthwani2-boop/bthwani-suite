...
- Service-wide runtime closure remains `PASS` after J-003/J-004/J-005/J-009 runtime proof and WLT-owned boundaries are proven together.
...
| `DSH-SAPI-P014-02` | `app-client` | `/app-client/cart` | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx` | review cart and hand off payment choice | `loading`, `error`, `blocked`, `retry` | `DSH_SLICE_L7_CLOSED` | WLT owns payment decision; backend auth middleware supports production mode (`DSH_AUTH_MODE=production` + Bearer token + auth service validation); app-client checkout transport can send Bearer token via `authToken` or `EXPO_PUBLIC_DSH_AUTH_BEARER_TOKEN` | none | none |
| `DSH-SAPI-P014-03` | `app-client` | `/app-client/orders` | `DshOrdersListScreen.tsx`; `DshTrackingScreen.tsx`; `DshConversationHubScreen.tsx`; `DshOrderIssueHubScreen.tsx`; `DshProxyHubScreen.tsx` | open timeline or issue workspace | `loading`, `error`, `success`, `offline`, `retry`, `blocked`, `cancelled` | `DSH_SLICE_L7_CLOSED` | WLT owns refund execution only | none | none |
| `DSH-SAPI-P014-04` | `app-partner` | `/app-partner/orders` | `OrdersInboxScreen.tsx`; `OperationScreens.tsx`; `DshPartnerOrderRejectionScreen.tsx` | accept, reject, or prepare an order | `loading`, `empty`, `error`, `success`, `offline`, `blocked`, `retry` | `DSH_SLICE_L7_CLOSED` | WLT only enters if later financial reversal is needed | none | none |
...
| `DSH-SAPI-P014-06` | `app-captain` | `/app-captain/orders` | `DshCaptainOrdersScreen.tsx`; `DshCaptainPickupDropoffScreen.tsx`; `DshCaptainMapScreen.tsx` | accept assignment and complete pickup | `loading`, `empty`, `error`, `success`, `retry` | `DSH_SLICE_L7_CLOSED` | WLT payout effects stay outside this slice | none | none |
| `DSH-SAPI-P014-07` | `app-captain` | `/app-captain/map` | `DshCaptainPoDSubmissionScreen.tsx`; `DshCaptainMapScreen.tsx` | submit proof of delivery or failure | `loading`, `success`, `error`, `retry` | `DSH_SLICE_L7_CLOSED` | WLT only owns downstream financial complaint/refund/payout execution | none | none |
