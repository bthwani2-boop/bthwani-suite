...
- client loyalty, subscription, favorites, promo, and marketing cluster
- client profile and preferences cluster
- partner subscription analytics cluster
- partner identity, intake, staff invite, and document admin cluster
- captain finance and tier cluster
- control-panel store items, zone admin, arrival bell, and misc spillover cluster

## Surface Exclusions

- `webapp` is out of current DSH first-service ownership
- `website` is out of current DSH first-service ownership
- `control-panel` is not allowed to absorb partner or captain execution work
- `app-field` is not allowed to become mandatory without stronger evidence than current packs show
...
Current lawful next work resumes at Phase 08 actor context exhaustive extraction.# 05_NON_GOALS
