...
import { buildStoreSearchCategories } from 'store-search-helpers';
import { getAllDeliveryModes } from 'store-formatting';
...
export function useStoreDerivedItems({
  ...
  const resolvedMenuItems = React.useMemo<DshStoreGetMenuItem[]>(() => menuItems ?? [], [menuItems]);
