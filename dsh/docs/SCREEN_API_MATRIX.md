...
| `DSH-SAPI-P014-03` | `app-client` | `/app-client/orders` | `DshOrdersListScreen.tsx`; `DshTrackingScreen.tsx`; `DshConversationHubScreen.tsx`; `DshOrderIssueHubScreen.tsx`; `DshProxyHubScreen.tsx` | open timeline or issue workspace | `loading`, `error`, `success`, `offline`, `retry`, `blocked`, `cancelled` | `PASS` | WLT owns refund execution only | none | none |
| `DSH-SAPI-P014-04` | `app-partner` | `/app-partner/orders` | `OrdersInboxScreen.tsx`; `OperationScreens.tsx`; `DshPartnerOrderRejectionScreen.tsx` | accept, reject, or prepare an order | `loading`, `empty`, `error`, `success`, `offline`, `blocked`, `retry` | `PASS` | WLT only enters if later financial reversal is needed | none | none |
...
| `DSH-SAPI-P014-06` | `app-captain` | `/app-captain/orders` | `DshCaptainOrdersScreen.tsx`; `DshCaptainPickupDropoffScreen.tsx`; `DshCaptainMapScreen.tsx` | accept assignment and complete pickup | `loading`, `empty`, `error`, `success`, `retry` | `PASS` | WLT payout effects stay outside this slice | none | none |
| `DSH-SAPI-P014-07` | `app-captain` | `/app-captain/map` | `DshCaptainPoDSubmissionScreen.tsx`; `DshCaptainMapScreen.tsx` | submit proof of delivery or failure | `loading`, `success`, `error`, `retry` | `PASS` | WLT only owns downstream financial complaint/refund/payout execution | none | none |
