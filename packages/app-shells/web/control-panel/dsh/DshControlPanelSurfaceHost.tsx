...
import { ControlPanelDshOrderDetailScreen, ControlPanelDshOrdersScreen } from '@bthwani/surfaces/dsh/control-panel/operations/dsh/orders';
...
import { ControlPanelOperationsOverviewDock } from './ControlPanelOperationsOverviewDock';
import { ControlPanelDshCatalogsDock } from './ControlPanelDshCatalogsDock';
import { ControlPanelDshOperationsDock } from './ControlPanelDshOperationsDock';
import { ControlPanelDshPartnersDock } from './ControlPanelDshPartnersDock';
import { ControlPanelDshSectionsDock } from './ControlPanelDshSectionsDock';
...
type DshWorkspaceId = 'overview' | 'orders' | 'order-detail' | 'reassign' | 'peak-mode' | 'arrival-bell';
...
export function DshControlPanelSurfaceHost({ section, subsection, workspace = 'overview', orderId }: DshControlPanelSurfaceHostProps) {
  const router = useRouter();
  const uiText = useUiText();
  const { language, setLanguage } = useDirection();
  const panelText = uiText.controlPanel;
  const dshText = (panelText.ui as typeof panelText.ui & { dsh: {
  ...
  const showOperationsWorkspace = activeSectionId === 'operations';
  const normalizedWorkspace = workspace === 'order-detail' ? 'orders' : workspace;
  const showOperationsOverview = showOperationsWorkspace && workspace === 'overview';
  const showDshOperationsDock = showOperationsOverview;

  const operationWorkspaceTabs = [
    { id: 'overview', label: dshText.overviewLabel, metaLabel: panelText.surfaceTitles.operations, active: normalizedWorkspace === 'overview' },
    { id: 'orders', label: dshText.ordersLabel, metaLabel: panelText.surfaceTitles.operations, active: normalizedWorkspace === 'orders' },
    { id: 'reassign', label: dshText.reassignLabel, metaLabel: panelText.surfaceTitles.operations, active: normalizedWorkspace === 'reassign' },
    { id: 'peak-mode', label: dshText.peakModeLabel, metaLabel: panelText.surfaceTitles.operations, active: normalizedWorkspace === 'peak-mode' },
    { id: 'arrival-bell', label: dshText.arrivalBellLabel, metaLabel: panelText.surfaceTitles.operations, active: normalizedWorkspace === 'arrival-bell' },
  ] as const;
    ...
    >
      <div className={styles.stageStack}>
        ...
        {showOperationsWorkspace ? (
          <BthWebSegmentedTabs
            ariaLabel={panelText.surfaceTitles.operations}
            items={operationWorkspaceTabs}
        ...
        {showOperationsOverview ? <ControlPanelOperationsOverviewDock /> : null}

        {showOperationsWorkspace && workspace === 'orders' ? (
          <ControlPanelDshOrdersScreen embedded showHeader={false} hubHref="/operations" operationsHref="/operations" />
        ) : null}

        {showOperationsWorkspace && workspace === 'order-detail' && orderId ? (
          <ControlPanelDshOrderDetailScreen embedded showHeader={false} orderId={orderId} hubHref="/operations" ordersHref="/operations/dsh/orders" />
        ) : null}

        {showOperationsWorkspace && workspace === 'reassign' ? (
          <ControlPanelDshReassignScreen embedded showHeader={false} hubHref="/operations" ordersHref="/operations/dsh/orders" />
        ) : null}

        {showOperationsWorkspace && workspace === 'peak-mode' ? (
          <ControlPanelDshPeakModeScreen embedded showHeader={false} hubHref="/operations" ordersHref="/operations/dsh/orders" />
        ) : null}

        {showOperationsWorkspace && workspace === 'arrival-bell' ? (
          <ControlPanelDshArrivalBellScreen embedded showHeader={false} hubHref="/operations" ordersHref="/operations/dsh/orders" />
        ) : null}
        ...
        {!showOperationsWorkspace ? (
          <ControlPanelDshSectionsDock activeSection={activeSectionId} activeSubSection={isControlSection ? subsection : undefined} />
        ) : null}

        {showDshOperationsDock ? <ControlPanelDshOperationsDock /> : null}

        {activeSectionId === 'partners' ? <ControlPanelDshPartnersDock /> : null}

        {activeSectionId === 'catalogs' ? <ControlPanelDshCatalogsDock /> : null}
