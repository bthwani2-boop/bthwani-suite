...
- **Current Cross-Surface Decision**: BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING.
...
> [!IMPORTANT]
> **DSH-SLICE-001 BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING**
> - DSH-SLICE-001 UI/UX + Visual evidence is locked.
> - Visibility gates (partner-readiness, catalog-approval, marketing-visibility) are fully implemented and verified via unit tests.
> - E2E Live DB curl validation and frontend UI integration verify proof.
...
|---|---|---|---|---|---|---|---|
| `app-client` | primary | Owns the visible discovery journey anchor | Visual & Runtime proof | app-client / GET /stores edge proof only; visibility gates implemented in backend and repository via unit tests | none | client | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| `app-partner` | dependency | Partner inventory/catalog readiness | Visual & Publishing-gate proof | `VR-L1-009 VISUAL_PASS (2026-06-02); backend endpoints and logic verified via unit tests` | visual captured / VISUAL_PASS | partner | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| `control-panel` (catalog governance) | dependency | Catalog approval & visibility governance | Catalog governance visual/runtime proof | `VR-L2-008 VISUAL_PASS; VR-L2-009 VISUAL_PASS (2026-06-03); backend endpoints and logic verified via unit tests` | visual captured / VISUAL_PASS | operations | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| `control-panel` (marketing visibility) | dependency | Marketing publish controls & visibility | Marketing visibility visual/runtime proof | `VR-L2-012 VISUAL_PASS (2026-06-03); backend endpoints and logic verified via unit tests` | visual captured / VISUAL_PASS | operations | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| shared DSH visibility/serviceability model | dependency | Direct shared logic for store exposure | Source & Runtime proof | `PASS` (bridge); logic and enums verified via unit tests | none | domain | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
...
| 3 | system | `shared model` | Evaluate visibility & serviceability | Rules + Store state | Visible to client | domain | N/A | N/A | Logic evaluation and gates visibility verified via unit tests | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| 4 | client | `app-client` | Open discovery feed & store | - | Store Details | preview | `loading`, `empty`, `error`, `success`, `offline` | open store, search inline | Visual pass (VR-L1-001/005/023) and runtime GET /stores visibility transition verified via unit tests | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
...
|---|---|---|---|---|---|---|---|---|
| `app-client` | `HomeScreen.tsx` | `dsh-home` | Open store | `dsh-store` | `loading`, `empty`, `error`, `success`, `offline` | `VR-L1-001` | visual captured / VISUAL_PASS; backend endpoints and logic verified via unit tests | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| `app-client` | `HomeScreen.tsx` | `dsh-home:inline-search` | Search inline | same page | `loading`, `empty`, `error`, `success`, `offline` | `VR-L1-023` | visual captured / VISUAL_PASS; backend endpoints and logic verified via unit tests | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| `app-client` | `StoreScreen.tsx` | `dsh-store` | View store details | same page | `loading`, `empty`, `error`, `success`, `offline` | `VR-L1-005` | visual captured / VISUAL_PASS; backend endpoints and logic verified via unit tests | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
...
|---|---|---|---|---|---|---|---|
| Discovery Stores | domain (Go backend) | `dsh/frontend/data` | app-client | Low | Local PostgreSQL Proven (`GET /stores`) | references/lean summaries | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| Partner Inventory | partner | `dsh/frontend/data` | app-partner, control-panel | High | Endpoints and DB writes verified via unit tests | references/lean summaries | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| Catalog Governance | operations | `dsh/frontend/data` | control-panel | High | Endpoints and DB writes verified via unit tests | references/lean summaries | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
| Marketing Visibility | operations | `dsh/frontend/data` | control-panel, app-client | Medium | Endpoints and DB writes verified via unit tests | references/lean summaries | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` |
...
- **Runtime Evidence per Surface**:
  ...
  - `app-partner` inventory catalog: `DSH-RUN-P014-04` (BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING: PATCH /stores/{id}/partner-readiness endpoint and DB updates verified via unit tests)
  - `control-panel` operations: `DSH-RUN-P014-07` (BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING: PATCH /stores/{id}/catalog-approval and /marketing-visibility verified via unit tests)
...
- **Runtime/API/L7** = BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING
- **partner readiness endpoint** = implemented and verified via unit tests
- **catalog approval endpoint** = implemented and verified via unit tests
- **marketing visibility endpoint** = implemented and verified via unit tests

- **Final Slice Decision**: `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING`.
- **Core Remaining Risks (Blockers)**:
  1. **E2E Live DB curl validation** remains pending to transition to fully proven.
  2. **Frontend UI integration** on partner and control-panel surfaces is not yet wired to these new endpoints.
- **Next Action**:
  Prepare subsequent frontend/transport bindings to hook these endpoints into the UI screens.
