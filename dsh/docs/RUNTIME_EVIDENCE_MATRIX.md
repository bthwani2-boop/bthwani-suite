...
| `DSH-RUN-P014-02` | app-client cart + checkout intent + checkout failure | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx` | `runtime-proven` | `needs-visual-evidence` | `NEEDS_BINDING_LATER` | `needs-visual-evidence` | live auth-service runtime proof + WLT runtime/security proof + visual proof pending | obtain live auth-service runtime proof and WLT E2E runtime proof |
| `DSH-RUN-P014-03` | app-client tracking + support | `OrdersTrackingScreens.tsx`; `OperationScreens.tsx` | `runtime-proven` | `needs-visual-evidence` | `NEEDS_RUNTIME_EVIDENCE` | `needs-visual-evidence` | deferred pending J-003 checkout/payment full closure | await J-003 checkout/payment full closure |
...
| `DSH-RUN-P014-06` | app-field onboarding + visit + readiness | `DshFieldStoresScreen.tsx`; `DshFieldStoreOnboardingScreen.tsx`; `DshFieldStoreVisitScreen.tsx`; `DshFieldReadinessEscalationScreen.tsx`; `dsh-field-visit-client.ts`; `POST /stores/{id}/field-visits` | `006A runtime-proven`; `006B postgres/http-runtime-proven`; `readiness local-state` | `006A_PASS__006B_IMPLEMENTED_RUNTIME_PROVEN__VISUAL_REBUILD_PENDING` | `006A_POST_STORES_RUNTIME_PROVEN`; `006B_POST_FIELD_VISITS_RUNTIME_PROVEN` | `006A_device_visual_evidence_captured`; `006B_http_201_evidence_captured` | updated app-field build is not installed on ADB device; TypeScript/Nx are blocked by Windows EPERM on node_modules; readiness outcomes remain downstream | rebuild/install current app-field, rerun TypeScript, capture field visit and evidence-detail screenshots |
...
| `DSH-RUN-P014-08` | control-panel finance bridge | `FinanceHubScreen.tsx`; `FinanceHubScreens.tsx`; `WltBoundaryBanner.tsx` | `preview`, `read-only bridge` | `BLOCKED_WITH_REASON` | `BLOCKED_BY_WLT` | `blocked-by-wlt` | settlement, payout, refund, commission, and ledger remain WLT-owned | WLT-owned runtime proof only; no DSH finance mutation |
