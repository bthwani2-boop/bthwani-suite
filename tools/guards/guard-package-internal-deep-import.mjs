...
const CONFIG_PATH = path.join(ROOT, 'tools', 'guards', 'guard-package-internal-deep-import.config.json');
...
function main() {
  ...
  const evidenceRoot = makeEvidenceRoot('GUARD_05_PACKAGE_INTERNAL_DEEP_IMPORT');
  const tracked = gitLsFiles().filter((file) => isTrackedSource(file));
  const findings = [];
  const allowed = new Set(config.allowedSpecifiers ?? []);
  const packageNames = config.packageNames ?? ['@bthwani/ui-kit', '@bthwani/surfaces'];
  const internalSegments = config.internalSegments ?? ['src', 'dist', '_internal', 'internal', 'components', 'foundation', 'tokens', 'patterns', 'primitives', 'service-owned', 'surface-owned'];
  ...
  for (const file of tracked) {
    ...
    for (const spec of specs) {
      for (const pkg of packageNames) {
        ...
        const isInternalSegment = internalSegments.includes(firstSegment);
        const isUiKitDeep = pkg === '@bthwani/ui-kit';
        const isSurfacesDeepInternal = pkg === '@bthwani/surfaces' && isInternalSegment;
        if (isUiKitDeep || isSurfacesDeepInternal || isInternalSegment) {
          findings.push({
            type: 'PACKAGE_INTERNAL_DEEP_IMPORT',
            severity: config.policy?.deepImportSeverity ?? 'warning',
            file,
            specifier: spec,
            package: pkg,
            reason: `Import bypasses public package surface or targets internal segment '${firstSegment}'.`,
          });
        }
      }
    }
  ...
  writeCsv(path.join(evidenceRoot, 'deep-import-findings.csv'), findings, ['type','severity','file','specifier','package','reason']);
  writeCsv(path.join(evidenceRoot, 'issues.csv'), findings, ['type','severity','file','specifier','package','reason']);

  const errors = findings.filter((item) => item.severity === 'error');
  const warnings = findings.filter((item) => item.severity !== 'error');
  const decision = errors.length ? 'BLOCKED_BY_PACKAGE_INTERNAL_DEEP_IMPORTS' : warnings.length ? 'READY_FOR_PACKAGE_INTERNAL_IMPORT_REVIEW_WITH_WARNINGS' : 'PASS_PACKAGE_INTERNAL_DEEP_IMPORT_GUARD';
  const summary = `# GUARD-05 — Package Internal Deep Import Guard\n\nDecision: ${decision}\nEvidenceRoot: ${toPosix(path.relative(ROOT, evidenceRoot))}\nGuardVersion: 1.0.0\n\n## Counts\n\n- SourceFilesScanned: ${tracked.length}\n- DeepImportFindings: ${findings.length}\n- Errors: ${errors.length}\n- Warnings: ${warnings.length}\n\n## Rule\n\nCHECK-only. Warning-first. No file changes.\n`;
  const zip = finishGuard({ evidenceRoot, decision, summary, evidence: {
    decision,
    repo: ROOT,
    evidence_root: evidenceRoot,
    guard: 'GUARD-05_PACKAGE_INTERNAL_DEEP_IMPORT',
    guard_version: '1.0.0',
    source_files_scanned: tracked.length,
    deep_import_findings: findings.length,
    errors: errors.length,
    warnings: warnings.length,
    check_only: true,
    no_delete: true,
  ...
  console.log('');
  console.log('GUARD-05 Package Internal Deep Import Guard complete.');
  console.log(`Decision: ${decision}`);
  console.log(`EvidenceRoot: ${evidenceRoot}`);
  console.log(`HandoffZip: ${zip.handoffZip}`);
  console.log(`DeepImportFindings: ${findings.length}`);
