        ...
        {LIVE_ORDERS.map((order, idx) => (
          <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1.4fr 1fr', gap: '16px', alignItems: 'start', padding: '16px', backgroundColor: '#fff', borderRadius: '8px', border: '1px solid rgba(10,47,92,0.08)', borderRight: order.status === 'متأخر' ? '4px solid #DC2626' : '4px solid transparent' }}>
            ...
            <div style={{ padding: '8px 10px', backgroundColor: 'rgba(10,47,92,0.03)', border: '1px solid rgba(10,47,92,0.07)', borderRadius: '6px' }}>
              <div style={{ fontSize: '12px', fontWeight: 700, color: '#0A2F5C', marginBottom: '2px' }}>توصية النظام: {order.suggestion.label}</div>
              <div style={{ fontSize: '11px', color: '#64748B', marginBottom: '4px' }}>السبب: {order.suggestion.reason}</div>
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', alignItems: 'center' }}>
                <ConfidenceBadge level={order.suggestion.confidence} />
