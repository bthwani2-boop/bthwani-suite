...
$SessionId = "CLOSE_EXACT_DUPLICATES_5-$((Get-Date).ToString('yyyyMMdd-HHmmss'))"
...
function Track-Deleted([string]$RelPath) {
  if (-not $Deleted.Contains($RelPath)) {
    $Deleted.Add($RelPath) | Out-Null
  }
}
...
function Close-OperationScreenDuplicate {
  ...
  foreach ($SurfaceRel in @($ClientRel, $CaptainRel)) {
    ...
    $WrapperLines.Add("// Compatibility wrapper: keeps existing surface import paths stable without duplicating UI code.") | Out-Null
    ...
    $WrappersReport.Add("WRAPPED: $SurfaceRel -> $SharedRel") | Out-Null
  }
...
function Make-Zip {
  $Zip = Join-Path $RunRoot "CLOSE_EXACT_DUPLICATES_5_NOW.zip"
  ...
  $Items = Get-ChildItem -LiteralPath $RunRoot -Force |
    Where-Object { $_.Name -notin @("CLOSE_EXACT_DUPLICATES_5_NOW.zip", "_HANDOFF.zip") }
...
$Summary = @(
  ...
  "4. Surface DshOperationScreen files become compatibility wrappers; UI JSX source becomes single shared file.",
  "",
  "## Guarantees",
  "",
  "- No exit.",
...
Write-Host "Upload: $RunRoot\CLOSE_EXACT_DUPLICATES_5_NOW.zip"
Write-Host ""
