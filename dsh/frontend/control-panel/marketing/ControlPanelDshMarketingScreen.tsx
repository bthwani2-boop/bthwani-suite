      ...
      case 'ticker': {
        ...
        return (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: '24px' }}>
            {/* Ticker Management List & Form */}
            <Box gap={4}>
              <Box gap={2}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <h3 style={{ color: '#0A2F5C', fontWeight: '800' }}>إدارة الرسائل النشطة</h3>
                  <button
                    onClick={() => {
                    ...
                    style={{ padding: '6px 12px', backgroundColor: '#0A2F5C', color: '#fff', borderRadius: '6px', border: 'none', fontWeight: '700', fontSize: '12px' }}
                  >
                    + إضافة رسالة جديدة
                  </button>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {tickers.map(ticker => (
                    <div
                      key={ticker.id}
                      onClick={() => setEditingTickerId(ticker.id)}
                      style={{
                        padding: '12px',
                        backgroundColor: editingTickerId === ticker.id ? '#F1F5F9' : '#fff',
                        borderRadius: '10px',
                        border: `1px solid ${editingTickerId === ticker.id ? '#0A2F5C' : 'rgba(10,47,92,0.06)'}`,
                        cursor: 'pointer',
                        display: 'flex',
                    ...
                    >
                      <Box gap={1}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          <span style={{
                            ...
                            padding: '2px 6px',
                            borderRadius: '4px',
                            backgroundColor: ticker.status === 'published' ? '#DCFCE7' : '#F1F5F9',
                            color: ticker.status === 'published' ? '#16A34A' : '#64748B',
                            fontWeight: '800'
                          }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px' }}>
                        <button
                          ...
                          title={ticker.status === 'published' ? 'إيقاف' : 'تفعيل'}
                        >
                          {ticker.status === 'published' ? '⏸️' : '▶️'}
                        </button>
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
              ...
              {editingTicker && (
                <div style={{ marginTop: '20px', padding: '20px', backgroundColor: '#fff', borderRadius: '12px', border: '1px solid rgba(10,47,92,0.08)' }}>
                  <h4 style={{ color: '#0A2F5C', fontWeight: '800', marginBottom: '16px', borderBottom: '1px solid #F1F5F9', paddingBottom: '8px' }}>تعديل محتوى الشريط</h4>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    <Box gap={1} style={{ gridColumn: 'span 2' }}>
                      ...
                      <textarea
                        ...
                        style={{ padding: '10px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '14px', minHeight: '60px', fontFamily: 'inherit' }}
                      />
                    </Box>
                    ...
                    <Box gap={1}>
                      ...
                      <select
                        value={editingTicker.audience}
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, audience: e.target.value as MarketingNewsTickerAudience });
                          refreshTickers();
                        }}
                    ...
                    <Box gap={1}>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, priority: e.target.value as MarketingNewsTickerPriority });
                          refreshTickers();
                        }}
                        style={{ padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '13px' }}
                      >
                    ...
                    <Box gap={1}>
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, source: e.target.value as MarketingNewsTickerSource });
                          refreshTickers();
                        }}
                        style={{ padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '13px' }}
                      >
                    ...
                    <Box gap={1}>
                      ...
                      <select
                        value={editingTicker.actionTarget}
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, actionTarget: e.target.value });
                          refreshTickers();
                        }}
                        style={{ padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '13px' }}
                      >
                    ...
                    <Box gap={1}>
                      <label style={{ fontSize: '11px', fontWeight: '800', color: '#64748B' }}>وقت البدء (ساعة)</label>
                      <input
                        type="number"
                        min="0"
                        max="23"
                        ...
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, openHour: parseInt(e.target.value) || 0 });
                          refreshTickers();
                        }}
                        style={{ padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '13px' }}
                      />
                    ...
                    <Box gap={1}>
                      <label style={{ fontSize: '11px', fontWeight: '800', color: '#64748B' }}>وقت الانتهاء (ساعة)</label>
                      <input
                        type="number"
                        min="0"
                        max="23"
                        ...
                        onChange={(e) => {
                          upsertMarketingTickerItem({ ...editingTicker, closeHour: parseInt(e.target.value) || 0 });
                          refreshTickers();
                        }}
                        style={{ padding: '8px', borderRadius: '8px', border: '1px solid #E2E8F0', fontSize: '13px' }}
                      />
            ...
            <Box gap={3}>
              ...
              <div style={{
                ...
                {/* Mock Client App Header */}
                <div style={{ backgroundColor: '#0A2F5C', height: '110px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  <div style={{
                    ...
                    <span style={{
                      fontSize: '9px',
                      fontWeight: '900',
                      backgroundColor: preview.isOpen ? '#FF500D' : '#64748B',
                      color: '#fff',
                      padding: '2px 6px',
                      borderRadius: '4px'
                    }}>
                ...
                <div style={{ flex: 1, backgroundColor: '#F8FAFC', padding: '16px' }}>
                  <div style={{ width: '100%', height: '150px', backgroundColor: '#E2E8F0', borderRadius: '16px', marginBottom: '16px' }} />
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
                    <div style={{ width: '60px', height: '60px', borderRadius: '12px', backgroundColor: '#CBD5E1' }} />
                    <div style={{ width: '60px', height: '60px', borderRadius: '12px', backgroundColor: '#CBD5E1' }} />
                    <div style={{ width: '60px', height: '60px', borderRadius: '12px', backgroundColor: '#CBD5E1' }} />
                    <div style={{ width: '60px', height: '60px', borderRadius: '12px', backgroundColor: '#CBD5E1' }} />
                  </div>
                  <div style={{ width: '100%', height: '100px', backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #E2E8F0' }} />
                </div>
              ...
              <div style={{ backgroundColor: '#F0F9FF', padding: '12px', borderRadius: '8px', border: '1px solid #BAE6FD' }}>
                <p style={{ fontSize: '11px', color: '#0369A1', margin: 0, fontWeight: '700' }}>
                   نافذة العرض: {preview.windowLabel}
                </p>
                <p style={{ fontSize: '10px', color: '#0369A1', marginTop: '4px', opacity: 0.8 }}>
                  * التغييرات تظهر لحظياً في المعاينة ويتم حفظها في المتجر المشترك.
                </p>
              </div>
            </Box>
          </div>
