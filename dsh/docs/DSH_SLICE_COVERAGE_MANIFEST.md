...
|---|---|---|---|---|---|---|
| J-001 | Store Discovery | Business | client, partner, operator | operations (catalog + marketing) | none | SLICE_GROUP_ACTIVE — see DSH-SLICE-001 + sub-slices 001A–001F |
| J-002 | Catalog Management | Business | partner, operator | catalog governance | none | PROPOSED — execution slices 002A–002G pending manifest |
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `DSH-SLICE-001A` | J-001 | Client sees live list of stores from API | client | app-client | shared visibility model | app-captain (N/A: no delivery); app-field (N/A: no field ops); WLT (N/A: no money) | open store; search inline | loading, empty, error, success, offline | dsh/frontend/data (preview); domain (visibility gate) | GET /stores — RUNTIME_PROVEN (DSH_SLICE001_LIVE_E2E-20260603-173059) | none | public/guest-safe | VR-L1-001, VR-L1-005, VR-L1-023 | DSH-RUN-P014-01 — PROVEN | All 3 visibility gate PASS + runtime matrix updated | PASS_WITH_WARNINGS — matrix update pending (CONTRA-001) | Resolve CONTRA-001 |
| `DSH-SLICE-001B` | J-001 | Client opens store details and browses inline | client | app-client | shared visibility model | app-captain (N/A); app-field (N/A); WLT (N/A) | open store; view details; search inline | loading, empty, error, success, offline | dsh/frontend/data (preview) | GET /stores/{id} — NOT_READY_FOR_API | none | public/guest-safe | yes — store detail screen | NEEDS_RUNTIME_EVIDENCE | Visual + runtime proof of store detail screen | DEFERRED_WITH_REASON — detail endpoint not designed | Classify after 001A closes |
| `DSH-SLICE-001C` | J-001 | Partner sets readiness; client_visible changes | partner | app-partner | app-client (serviceability reader), control-panel | app-captain (N/A); app-field (N/A); WLT (N/A) | update readiness; toggle ready/paused | loading, success, offline | dsh/frontend/data (preview); domain (readiness logic) | PATCH /stores/{id}/partner-readiness — FRONTEND_TRANSPORT_BOUND__SCREEN_PROOF_PENDING | none | partner auth | VR-L1-009 VISUAL_PASS (2026-06-02) | screen proof after pressing readiness button | Screen proof captured with Go API running | FIX_REQUIRED — RUNTIME_EVIDENCE_MATRIX not updated (CONTRA-001) | Capture screen proof; update matrix |
| `DSH-SLICE-001D` | J-001 | Operator approves catalog; client_visible changes | operator | control-panel (catalogs) | app-client (serviceability reader), app-partner | app-captain (N/A); app-field (N/A); WLT (N/A) | approve catalog; reject catalog | loading, success, error | domain | PATCH /stores/{id}/catalog-approval — FRONTEND_TRANSPORT_BOUND__SCREEN_PROOF_PENDING | none | operator auth | VR-L2-008, VR-L2-009 VISUAL_PASS | screen proof after pressing catalog approval | Screen proof captured with Go API running | FIX_REQUIRED — screen proof pending; CONTRA-002 | Capture screen proof; update matrix |
| `DSH-SLICE-001E` | J-001 | Operator sets marketing visibility; client_visible changes | operator | control-panel (marketing) | app-client (serviceability reader) | app-captain (N/A); app-field (N/A); WLT (N/A) | set active (visible); set inactive | loading, success, error | domain | PATCH /stores/{id}/marketing-visibility — FRONTEND_TRANSPORT_BOUND__SCREEN_PROOF_PENDING | none | operator auth | VR-L2-012 VISUAL_PASS | screen proof after pressing visibility toggle | Screen proof captured with Go API running | FIX_REQUIRED — screen proof pending; CONTRA-002 | Capture screen proof; update matrix |
| `DSH-SLICE-001F` | J-001 | Cross-surface final visibility proof: all 3 gates affect GET /stores together | operator + partner + client | app-client + app-partner + control-panel | shared visibility model | app-captain (N/A); app-field (N/A); WLT (N/A) | (implicit — derived from 001C/D/E) | all states from 001A–001E | domain | GET /stores diff after each PATCH gate | none | all roles | combined visual evidence (all 3 gates) | GET /stores response diff before/after each PATCH | All 3 PATCH gates affect GET /stores in a single session | BLOCKED_WITH_REASON — depends on 001C/D/E screen proof | Close 001C, 001D, 001E first |
| `DSH-SLICE-002A` | J-002 | Partner identifies and manages product identity (SKU/GTIN/barcode) | partner | app-partner | control-panel (catalog governance) | app-captain (N/A); app-field (N/A); WLT (N/A) | edit product; set SKU; scan barcode | loading, success, error, offline | dsh/frontend/data (preview); partner owns inventory source | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | partner auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002B` | J-002 | Partner manages category structure (main/sub/facet) | partner | app-partner | control-panel (catalog governance) | app-captain (N/A); app-field (N/A); WLT (N/A) | set main category; set sub-category; set facet | loading, success, error, offline | dsh/frontend/data (preview) | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | partner auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002C` | J-002 | Media ownership: mediaKey assignment and image governance | partner, operator | app-partner + control-panel | shared media governance | app-captain (N/A); WLT (N/A) | assign mediaKey; upload image; reject image | loading, success, error | domain (mediaKey rules); dsh/frontend/media-fixtures | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | partner + operator auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002D` | J-002 | Partner local overrides boundary: partner can override within allowed scope | partner | app-partner | control-panel (override governance) | app-captain (N/A); WLT (N/A) | apply override; remove override | loading, success, error | partner owns overrides; domain owns base | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | partner auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002E` | J-002 | Catalog approval / rejection / needs-fix workflow | operator | control-panel (catalogs) | app-partner (submitter), app-client (consumer) | app-captain (N/A); WLT (N/A) | approve; reject; request fix | loading, success, error | domain | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | operator auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002F` | J-002 | Listing visibility / publication boundary | partner, operator | app-partner + control-panel | app-client (consumer) | app-captain (N/A); WLT (N/A) | publish listing; unpublish listing; set visibility | loading, success, error | domain (visibility gate) | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | partner + operator auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-002G` | J-002 | Catalog conflict / duplicate / audit handling | operator | control-panel (catalogs) | app-partner | app-captain (N/A); app-field (N/A); WLT (N/A) | flag conflict; resolve duplicate; view audit log | loading, success, error | domain | CANDIDATE_AFTER_VISUAL_AND_RUNTIME_PROOF | none | operator auth | yes | NEEDS_RUNTIME_EVIDENCE | Slice manifest complete; visual + runtime proof | DEFERRED_WITH_REASON — slice manifest not yet created | Create slice manifest |
| `DSH-SLICE-003A` | J-003 | Client reviews cart and confirms serviceability lock | client | app-client (cart) | shared serviceability model | app-captain (N/A: not yet assigned); WLT (N/A: payment not started) | view cart; remove item; proceed to checkout | loading, error, blocked, retry | preview/local-state | BLOCKED_BY_WLT/AUTH | WLT starts at payment intent | client auth | yes | NEEDS_RUNTIME_EVIDENCE | WLT/auth runtime proof exists; serviceability lock proven | BLOCKED_WITH_REASON — blocked by WLT/auth proof | Do not start until WLT/auth runtime proof exists |
| `DSH-SLICE-003B` | J-003 | Client initiates checkout intent | client | app-client (checkout) | WLT bridge | app-captain (N/A); app-field (N/A) | confirm order intent; cancel | loading, error, blocked | preview/local-state | BLOCKED_BY_WLT/AUTH | WLT bridge starts | client auth | yes | NEEDS_RUNTIME_EVIDENCE | WLT/auth runtime proof; order intent API proven | BLOCKED_WITH_REASON — blocked by WLT/auth proof | Do not start until WLT/auth runtime proof exists |
| `DSH-SLICE-003C` | J-003 | WLT payment bridge: client selects payment method and WLT processes | WLT | WLT surfaces | app-client (payment trigger) | app-captain (N/A); app-field (N/A) | select wallet; confirm payment | loading, success, failure | WLT-owned | BLOCKED_BY_WLT | full WLT ownership | WLT-managed | yes | NEEDS_WLT_RUNTIME_PROOF | WLT payment proven E2E | BLOCKED_WITH_REASON — WLT owns; no DSH finance mutation | Keep blocked until WLT provides runtime proof |
| `DSH-SLICE-003D` | J-003 | Order creation handoff to partner after WLT payment | partner, system | app-partner (intake) | app-client (order placed), control-panel (ops monitor) | app-captain (N/A: not assigned yet) | accept new order notification | loading, success, error | domain | CANDIDATE_AFTER_WLT_PROOF | WLT provides order ID | partner auth | yes | NEEDS_RUNTIME_EVIDENCE | WLT proof + partner intake API proven | BLOCKED_WITH_REASON — depends on 003B/003C | Close 003B + 003C first |
| `DSH-SLICE-003E` | J-003 | Payment/order failure and support path | client, operator | app-client + control-panel (support) | WLT (failure signal) | app-captain (N/A) | retry payment; contact support; view failure reason | loading, error, retry | WLT/domain | BLOCKED_BY_WLT | WLT owns failure signal | client + operator auth | yes | NEEDS_RUNTIME_EVIDENCE | WLT failure signal + support path proven | BLOCKED_WITH_REASON — depends on 003C | Close 003C first |
