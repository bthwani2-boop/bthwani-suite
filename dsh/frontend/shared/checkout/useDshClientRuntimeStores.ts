...
import {
  ...
  type DshListDiscoveryStoresResponse,
} from '../stores';
import {
  clientVisibleDiscoveryStores as initialDiscoveryStores,
  clientVisibleHomeStores as initialHomeStores,
} from './dsh-client-binding.contracts';
import { getDshDiscoveryStoresRuntimeClient } from '../runtime/ui-only-runtime-clients';
...
export function useDshClientRuntimeStores() {
  ...
  React.useEffect(() => {
    ...
    client.listDiscoveryStores().then((response: DshListDiscoveryStoresResponse) => {
      if (cancelled) return;
      setRuntimeBridge(
        resolveDshDiscoveryStoresBridge({
          response,
