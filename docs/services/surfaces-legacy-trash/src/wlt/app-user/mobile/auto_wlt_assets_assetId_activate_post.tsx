// Auto-generated screen for wlt_assets_assetId_activate_post
...
export const auto_wlt_assets_assetId_activate_post: React.FC = () => {
  const { t } = useI18n();
  const [state, setState] = useState<ScreenState>('content');
  const handleActivate = () => setState('loading');
  if (state === 'content') {
    return (
      <ScreenWrapper state="content">
        <View style={styles.box}>
          <Text style={styles.title}>{t('surfaces.تفعيل_الأصل')}</Text>
          <TouchableOpacity style={styles.btn} onPress={handleActivate}>
            <Text style={styles.btnText}>{t('surfaces.تفعيل')}</Text>
          </TouchableOpacity>
        </View>
      </ScreenWrapper>
    );
  ...
  return (
    <ScreenWrapper
      state="loading"
      loadingMessage={t('surfaces.جاري_التفعيل')}
      screenName="auto_wlt_assets_assetId_activate_post"
      operationName="wlt_assets_assetId_activate_post"
    />
  );
};
...
export default auto_wlt_assets_assetId_activate_post;
