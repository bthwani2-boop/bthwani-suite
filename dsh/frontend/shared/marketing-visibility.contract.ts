...
function resolveMarketingPublishStage(input: MarketingVisibilityInput, approvalStatus: MarketingVisibilityApprovalStatus): string | undefined {
  if (input.publishStage) {
    return input.publishStage;
  }

  if (approvalStatus === 'client-visible') {
    return 'client-visible';
  }

  if (approvalStatus === 'catalog-adopted') {
    return 'catalog-adopted';
  }

  if (approvalStatus === 'marketing-approved') {
    return 'marketing-approved';
  }

  if (approvalStatus === 'partner-approved') {
    return 'partner-approved';
  }

  if (approvalStatus === 'active' || approvalStatus === 'published') {
...
function resolveMarketingRouteHint(input: MarketingVisibilityInput): MarketingRouteHint {
  ...
  if (input.targetSurface === 'partner-promotions') {
    return 'partner-promotions';
  }

  if (input.targetSurface === 'control-panel') {
    return 'campaigns';
  }

  if (input.targetSurface === 'signals') {
    return 'signals';
  }
...
function resolvePartnerBlockedReason(status: DshPartnerActivationStatus | undefined): string {
  if (!status) {
    return 'العنصر مرتبط بمتجر لكن حالة ظهور الشريك للعملاء غير مؤكدة بعد.';
  }
...
function resolveProductBlockedReason(status: DshProductIdentityApprovalStatus | undefined): string {
  if (!status) {
    return 'العنصر مرتبط بمنتج لكن حالة نشره للعملاء غير مؤكدة بعد.';
  }
...
function resolveDisplayStatus(
  ...
  if (targetSurface === 'partner-promotions' || targetSurface === 'control-panel' || targetSurface === 'signals' || targetSurface === 'catalog') {
    if (approvalStatus === 'archived' || approvalStatus === 'expired' || approvalStatus === 'exhausted') {
      return 'hidden';
    }
