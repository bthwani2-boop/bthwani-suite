/**
 * WLT DSH Maker-Checker Contract — Preview Type Definitions
 *
 * Full workflow lifecycle for financial approval chains.
 * The Maker prepares/registers the entry.
 * The Checker independently reviews and approves or rejects.
 * WLT runtime executes only after full approval chain.
 * DSH has NO real approval authority — it prepares and views only.
 *
...
export type WltMakerCheckerRole =
  | 'maker'
  | 'checker'
  | 'wlt_processor';
...
export const WLT_MAKER_CHECKER_TRANSITIONS: Readonly<
  Record<WltMakerCheckerState, readonly TransitionDef[]>
> = {
  draft: [
    { allowedRole: 'maker', nextState: 'prepared_by_maker', permissionRequired: 'finance:prepare' },
  ],
  prepared_by_maker: [
    { allowedRole: 'checker', nextState: 'under_review', permissionRequired: 'finance:review' },
  ],
  under_review: [
  ...
  checked_by_checker: [
    { allowedRole: 'wlt_processor', nextState: 'approved_for_wlt', permissionRequired: 'wlt:execute' },
  ],
  approved_for_wlt: [],
  rejected_by_checker: [
    { allowedRole: 'maker', nextState: 'draft', permissionRequired: 'finance:prepare' },
  ],
  blocked_by_variance: [
    { allowedRole: 'maker', nextState: 'draft', permissionRequired: 'finance:prepare' },
  ],
...
export const WLT_RBAC_PREVIEW_ROLES: readonly WltRbacPreviewRole[] = [
  {
    roleId: 'finance-maker',
    label: 'مُعِدّ مالي (Maker)',
    permissions: ['finance:prepare'],
    description: 'يُسجّل القيود ويجهّز الأدلة. لا صلاحية اعتماد.',
    isPreview: true,
  },
  {
    roleId: 'finance-checker',
    label: 'مُدقق مالي (Checker)',
    permissions: ['finance:review', 'finance:approve', 'finance:reject'],
    description: 'يراجع القيود باستقلالية تامة. مستقل عن Maker. يملك صلاحية الاعتماد أو الرفض.',
    isPreview: true,
  },
  {
    roleId: 'finance-day-closer',
    label: 'مُغلق اليوم المالي',
    permissions: ['finance:prepare', 'finance:review', 'finance:close-day'],
    description: 'يُحضّر حزمة الإغلاق بعد اكتمال كل الشروط. لا يعتمد منفردًا.',
    isPreview: true,
  },
  {
    roleId: 'wlt-processor',
    label: 'محرك WLT (آلي)',
    permissions: ['wlt:execute'],
    description: 'WLT runtime فقط. يُنفّذ الترحيل بعد اكتمال سلسلة الاعتمادات.',
    isPreview: true,
  },
] as const;
...
export function buildWltMakerCheckerRecord(params: {
  entryId: string;
  workflowState: WltMakerCheckerState;
}): WltMakerCheckerRecord {
  const transitions = getWltNextTransitions(params.workflowState);
  const nextRole = transitions[0]?.allowedRole ?? 'none';
  const permission = transitions[0]?.permissionRequired ?? 'finance:prepare';
