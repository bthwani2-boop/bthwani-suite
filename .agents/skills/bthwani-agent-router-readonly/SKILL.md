---
name: bthwani-agent-router-readonly
description: Safe advisor transformed from Agents Orchestrator. Chooses the smallest suitable skill; does not run multi-agent workflows.
---

# BThwani Agent Router Readonly - BThwani Safe Skill
...
Mode: transformed safe skill router
...
- Select the minimum sufficient BThwani skill for the task.
- Reduce token waste by avoiding broad or overlapping skills.
- Reject unsafe raw agent use.
- Recommend read-only, advisory, implementation, review, or evidence path.

## Forbidden Use

- Do not automatically delegate to multiple agents.
- Do not expand the task scope.
- Do not override governance or user constraints.
...
- state rejected broader skills
- state evidence needed before execution
