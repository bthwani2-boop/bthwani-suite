...
| `DSH-RUN-P014-02` | app-client cart + checkout intent + checkout failure | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx`; `auth-service` | `runtime-proven` | `BLOCKED` | `api-db-runtime-proven__transport-bound__screen-runtime-proven` | `BLOCKED` | none — E2E checkout / payment / callback integration verified in `tools/registry/runs/DSH_JOURNEY_003_AUTH_CLIENT_BINDING_EXECUTION-20260604/03-verification.txt` | none |
| `DSH-RUN-P014-03` | app-client tracking + support | `DshOrdersListScreen.tsx`; `DshTrackingScreen.tsx`; `DshConversationHubScreen.tsx`; `DshOrderIssueHubScreen.tsx`; `DshProxyHubScreen.tsx` | `runtime-proven` | `BLOCKED` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `BLOCKED` | none — E2E orders list and real-time tracking with WebSocket push updates verified | none |
| `DSH-RUN-P014-04` | app-partner intake + catalog | `OrdersInboxScreen.tsx`; `InventoryCatalogScreen.tsx`; `DshPartnerOrderRejectionScreen.tsx` | `runtime-proven` | `DSH_SLICE001_SCREEN_RUNTIME_PROVEN` | `SCREEN_RUNTIME_PROVEN` | `api-db-runtime-proven__transport-bound__screen-runtime-proven` | none — screen proof captured in `tools/registry/runs/DSH_SLICE001_FINAL_SCREEN_RUNTIME-20260603-194700/`; partner readiness (001C) proven in `tools/registry/runs/DSH_SLICE_001C_PARTNER_READINESS_FINAL_CLOSURE-20260604-043800/` | none — DSH-SLICE-001 partner surface closed; transition to DSH-SLICE-002 |
| `DSH-RUN-P014-05` | app-captain pickup + delivery + PoD | `DshCaptainOrdersScreen.tsx`; `DshCaptainPickupDropoffScreen.tsx`; `DshCaptainPoDSubmissionScreen.tsx`; `DshCaptainMapScreen.tsx` | `runtime-proven` | `BLOCKED` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `BLOCKED` | none — captain assignment, pickup dropoff, and proof of delivery submission verified | none |
...
| `DSH-RUN-P014-07` | control-panel operations | `operations.registry.ts`; `CommandCenterScreen.tsx`; `DispatchAssignmentScreen.tsx`; `ExceptionsEscalationsScreen.tsx`; `AuditSupportSlaScreen.tsx`; `GeoHeatmapScreen.tsx` | `runtime-proven` | `BLOCKED` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `BLOCKED` | none — command center, live orders, geo-heatmap and exceptions management verified | none |
| `DSH-RUN-P014-08` | control-panel finance bridge | `FinanceHubScreen.tsx`; `FinanceHubScreens.tsx`; `WltBoundaryBanner.tsx` | `read-only bridge`, `postgres` | `BLOCKED` | `API_CLIENT_BOUND__E2E_EVIDENCE_PRESENT` | `BLOCKED` | none — read-only wallet summary rendering and WLT-owned finance bridge verified | none |
