# Screen Contract - snd_request_status_update

1. Screen Identity: SCR_5
2. Purpose: update SND request status.
3. Actor: client
4. Surface: app-client
5. Canonical Route: /snd/requests/status/update
6. Entry Points: requests list.
7. Exit Points: requests list.
8. Primary CTA: submit_update
9. Secondary Actions: retry, back.
10. Required Data: status update payload.
11. Displayed Blocks: status controls and confirmation.
12. Section Order: header -> controls -> actions.
13. Interaction Rules: one canonical chain.
14. Validation Rules: payload validation.
15. Empty / Error / Loading / Offline / Disabled States: mandatory.
16. Dependent Operations: snd_request_status_update
17. Required UI-Kit Pieces: form controls, state shells.
18. Local vs Shared Ownership: shared ui-kit primitives.
19. Files To Create / Files To Touch: packages/surfaces/src/snd/snd_request_status_update.tsx
20. Acceptance Gate: gate_a_b_c_passed
21. Deferred Items: W08/W09.
22. Notes from donor extraction: services/snd/governance/SND_OPERATION_CATALOG.csv
