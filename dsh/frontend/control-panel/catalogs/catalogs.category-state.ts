...
export type UseCatalogCategoryStateParams = {
  pushPreviewProposal: (p: CatalogPreviewProposal) => void;
  products: readonly { categoryPath: { main: string; sub?: string } }[];
};

export function useCatalogCategoryState({
  pushPreviewProposal,
  products,
}: UseCatalogCategoryStateParams) {
  ...
  const handleAddMainCategory = React.useCallback(() => {
    ...
    setPreviewCategories((prev) => [...prev, createPreviewMainCategory(id, label, formSubtitle.trim())]);
  ...
  const handleDeleteNode = React.useCallback((
    ...
    setPreviewCategories((prev) => {
      if (type === 'main') return prev.filter(c => c.id !== mainId);
      if (type === 'sub') return prev.map(c => c.id === mainId ? { ...c, subcategories: c.subcategories.filter(s => s.id !== subId) } : c);
      if (type === 'mainClassif') return prev.map(c => c.id === mainId ? {
        ...c,
    ...
    if (type === 'main' && activeMainCategory?.id === mainId) {
      setActiveMainCategory(null); setActiveSubCategory(null);
      setActiveMainClassifId(null); setActiveSubClassifId(null);
    } else if (type === 'sub' && activeSubCategory?.id === subId) {
      setActiveSubCategory(null); setActiveMainClassifId(null); setActiveSubClassifId(null);
    ...
    setEditingEntry(null); setCatError(null);
  ...
  const handleResetCategoryPreview = React.useCallback(() => {
    ...
    setAddingMainClassifUnder(null); setAddingSubClassifUnder(null);
    setFormLabel(''); setFormSubtitle(''); setCatError(null);
    setEditingEntry(null); setActiveMainClassifId(null); setActiveSubClassifId(null);
  }, []);
  ...
  const handleStartCatEdit = React.useCallback((
    ...
    if (type === 'main') { setEditLabel(cat.label); setEditSubtitle(cat.subtitle); }
      ...
      if (!classif) return;
      setEditLabel(classif.label); setEditSubtitle('');
    } else {
      const sub = cat.subcategories.find((s) => s.id === subId);
      const classif = sub?.mainClassifications?.find(c => c.id === mainClassifId);
      const subc = classif?.subClassifications?.find(s => s.id === subClassifId);
      if (!subc) return;
      setEditLabel(subc.label); setEditSubtitle('');
    }
    ...
    setPreviewCategories((prev) => {
      if (editingEntry.type === 'main') {
        ...
        return prev.map((cat) => {
          ...
          return { ...cat, subcategories: cat.subcategories.map((s) => s.id === editingEntry.subId ? { ...s, label, subtitle: editSubtitle.trim() } : s) };
        });
        ...
        return prev.map((cat) => {
          ...
          return {
            ...
            subcategories: cat.subcategories.map(sub => {
              ...
              return { ...sub, mainClassifications: (sub.mainClassifications || []).map(mc => mc.id === editingEntry.mainClassifId ? { ...mc, label } : mc) };
            })
          };
        });
      } else {
            ...
            subcategories: cat.subcategories.map(sub => {
              ...
              return {
                ...
                mainClassifications: (sub.mainClassifications || []).map(mc => {
                  ...
                  return { ...mc, subClassifications: (mc.subClassifications || []).map(sc => sc.id === editingEntry.subClassifId ? { ...sc, label } : sc) };
                })
              };
            })
          };
