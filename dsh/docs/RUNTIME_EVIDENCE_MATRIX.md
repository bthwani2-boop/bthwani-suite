...
|---|---|---|---|---|---|---|---|---|
| `DSH-RUN-P014-01` | app-client discovery + storefront | `HomeScreen.tsx`; `SearchScreen.tsx`; `StoreScreen.tsx`; `DshClientSurface.tsx`; typed client bridge | `preview-fallback`, `props-driven`, `local-state`, `typed-client-boundary` | `BATCH_9A_GO_BACKEND_SKELETON_READY_FOR_POSTGRES` | `FRONTEND_TRANSPORT_NOT_BOUND` | `runtime-unproven` | no real `GET /stores` response has reached the screen; PostgreSQL proof, frontend transport, and E2E request/response evidence are still missing | Batch 9B local PostgreSQL runtime for `GET /stores` only |
...
| `DSH-RUN-P014-09` | backend + domain + OpenAPI | `dsh/backend`; `dsh/domain`; `dsh/dsh.openapi.yaml` | `go-skeleton`, `memory-repository`, `openapi-aligned` | `BATCH_9A_GO_BACKEND_SKELETON_READY_FOR_POSTGRES` | `GET_STORES_HANDLER_ONLY` | `runtime-unproven` | PostgreSQL persistence, Docker Compose, migrations, seed, frontend transport, and E2E UI proof are still missing | Batch 9B may replace the temporary memory repository with local PostgreSQL runtime for `GET /stores` only |
