...
| `DSH-RUN-P014-02` | app-client cart + checkout intent + checkout failure | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx`; `DshCheckoutFailureScreen.tsx` | `runtime-proven` | `BLOCKED_WITH_REASON` | `API_CLIENT_BOUND__NEEDS_RUNTIME_EVIDENCE` | `BLOCKED_WITH_REASON` | auth service backend now live (`dsh/backend/cmd/auth-service`, port 8092, migration 027, dev tokens seeded); WLT E2E payment callback runtime remains pending | run smoke: GET /auth/session with dev-client-token-001; then test checkout → WLT payment session E2E |
...
| `DSH-RUN-P014-06` | app-field onboarding + visit + readiness | `DshFieldStoresScreen.tsx`; `DshFieldStoreOnboardingScreen.tsx`; `DshFieldStoreVisitScreen.tsx`; `DshFieldReadinessEscalationScreen.tsx`; `dsh-field-visit-client.ts`; `POST /stores/{id}/field-visits` | `006A-E pending E2E` | `BLOCKED_WITH_REASON` | `API_CLIENT_BOUND__NEEDS_RUNTIME_EVIDENCE` | `BLOCKED_WITH_REASON` | auth service backend now live (port 8092, dev-field-token-001 seeded); remaining: E2E smoke with real Bearer token against local DSH + Postgres | run: POST /stores/{id}/field-visits with Authorization: Bearer dev-field-token-001 against local Go API |
...
| `DSH-RUN-P015-01` | auth service (all surfaces) | `dsh/backend/cmd/auth-service/main.go`; `dsh/backend/migrations/027_auth_sessions.sql`; `auth.openapi.yaml` | `backend-implemented`, `postgres-migration-ready`, `dev-tokens-seeded` | `AUTH_SERVICE_BACKEND_LIVE__STARTUP_WIRED` | `BACKEND_IMPLEMENTED__STARTUP_SCRIPT_WIRED` | `implementation-complete__runtime-smoke-pending` | runtime smoke test against live Postgres still needed; E2E proof (GET /auth/session with dev tokens, POST /auth/login, GET /auth/permissions) | start auth service via START_LOCAL_LIVE_TEST.ps1 and smoke GET http://localhost:8092/auth/session with dev-client-token-001 |
