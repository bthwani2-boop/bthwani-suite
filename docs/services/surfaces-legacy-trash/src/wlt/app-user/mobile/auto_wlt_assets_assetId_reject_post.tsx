// Auto-generated screen for wlt_assets_assetId_reject_post
...
export const auto_wlt_assets_assetId_reject_post: React.FC = () => {
  const { t } = useI18n();
  const [state, setState] = useState<ScreenState>('content');
  const handleReject = () => setState('loading');
  if (state === 'content') {
    return (
      <ScreenWrapper state="content">
        <View style={styles.box}>
          <Text style={styles.title}>{t('surfaces.رفض_الأصل')}</Text>
          <TouchableOpacity style={styles.btn} onPress={handleReject}>
            <Text style={styles.btnText}>{t('surfaces.رفض')}</Text>
          </TouchableOpacity>
        </View>
      </ScreenWrapper>
    );
  ...
  return (
    <ScreenWrapper
      state="loading"
      loadingMessage={t('surfaces.جاري_الرفض')}
      screenName="auto_wlt_assets_assetId_reject_post"
      operationName="wlt_assets_assetId_reject_post"
    />
  );
};
...
export default auto_wlt_assets_assetId_reject_post;
