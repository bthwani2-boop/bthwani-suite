# Invoke-LocalStack.ps1
# Canonical root-level Docker management script for the BThwani local stack.
# Runs from the workspace root. Handles conflict resolution, health checks, and HTTP smoke tests.
#
# Usage: Edit the variables below and run the script directly, or pass overrides via the
#        $Action / $Stack environment variables before sourcing.
#
# Actions : up | restart | status | logs | stop | clean-stale
# Stacks  : core | dsh | wlt
