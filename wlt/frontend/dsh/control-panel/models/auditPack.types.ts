/**
 * WLT DSH Audit Pack — types + builder.
...
export type WltAuditPackStatus =
  ...
  | 'submitted_to_wlt' | 'executed' | 'rejected';

export type WltAuditEventType =
  | 'daily-close-initiated' | 'entry-prepared' | 'entry-reviewed' | 'entry-approved'
  | 'entry-rejected' | 'variance-detected' | 'evidence-uploaded' | 'gate-opened'
  | 'gate-closed' | 'pack-submitted' | 'wlt-executed';
...
export function buildWltAuditPackPreview(params: {
  ...
  const allApproved = params.entries.every((e) => e.workflowState === 'approved' || e.workflowState === 'checked_by_checker');
  ...
  return {
    closingRunId: `CLOSE-PRV-${dateSlug}`,
    ...
    exceptions: params.entries
      ...
      .map((e) => ({
        ...
        severity: (e.evidenceStatus === 'missing' || e.workflowState === 'blocked_wlt' ? 'blocking' : 'warning') as WltAuditException['severity'],
      })),
