/**
 * FieldBottomNavigationBar — 3-4 Tabs Navigation
...
const NS = 'mobile.app-field.FieldBottomNavigationBar';

export type FieldState = 'idle' | 'on_task' | 'issue';

interface FieldBottomNavigationBarProps {
...
export const FieldBottomNavigationBar: React.FC<
  FieldBottomNavigationBarProps
> = ({ currentScreen, onNavigate, fieldState, notificationCount = 0 }) => {
  const { rowStyle, t } = useDirection();
  const FIELD_TABS: TabConfig[] = React.useMemo(
    () => [
      { key: 'tasks', label: t(`${NS}.tasks`), icon: 'list', screen: 'Home' },
      { key: 'map', label: t(`${NS}.map`), icon: 'map', screen: 'FieldMap' },
      {
        key: 'profile',
        label: t(`${NS}.account`),
        icon: 'person',
  ...
  const isActive = (tab: TabConfig): boolean => {
    if (
      tab.screen === 'Home' &&
      (currentScreen === 'Home' ||
        currentScreen.startsWith('field_') ||
        currentScreen.startsWith('arb_'))
    ) {
