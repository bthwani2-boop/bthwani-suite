...
| Capability | Home Banner Carousel / Promos / Marketing Block |
| Related surfaces | `app-client` proven final display surface; `control-panel` marketing proven campaign/banner governance surface; `control-panel` partners proven partner/store readiness and marketing handoff surface; `app-partner` proven offer/product/store promotion-intent surface; `dsh/media-fixtures` provides seed media references only |
...
| Active evidence | `DshSurfaceHost.tsx` injects `resolvePublishedHomePromos()` into `DshHomeGetScreen`; `DshHomeGetScreen.tsx` maps promos into `BannerCarousel` and resolves `main_category`, `sub_category`, `store`, `store_category`, `product`, `subscription`, and `external` press paths; `ControlPanelDshMarketingScreen.tsx` renders `BannersCommandDeckScreen` for banner publish/order/action governance; `ControlPanelDshPartnerApprovalsScreen.tsx` now acts as a partner intake hub: it exposes `offer-approval`, `partner-review`, and `marketing-review` queues, embeds `DshPartnerPromotionEligibilityScreen.tsx`, and adds source-to-surface lanes for entry, orders, operations, inventory, finance, growth, settings, and support before downstream routing; `DshPartnerConsoleScreen.tsx` routes `activeSection === 'analytics'` to `AnalyticsGrowthMarketingWorkspaceContent`, where the partner sees active/suggested campaigns and local CTAs such as `إنشاء عرض سريع`; all of this remains preview/local-state evidence, not runtime truth |
...
| Next action | keep this as docs-only baseline; next bounded implementation step is to prove source authority and shared publish semantics between app-client promo consumption, control-panel marketing governance, control-panel partner readiness, and app-partner offer intent before any contract or runtime claim |
...
|---|---|---|---|---|---|---|
| app-partner | offer/product/store promotion intent; participates in promotion intent, not final banner design | `dsh/frontend/app-partner/console/screens/DshPartnerConsoleScreen.tsx` -> analytics section | The promotion intent surface now lives inside `DshPartnerConsoleScreen.tsx` as part of the analytics workspace, with candidate selection and CTA flow folded into the same screen | OFFER_SOURCE | CANDIDATE_SCREEN | prove route/runtime or request persistence later; keep the analytics workspace as the single partner context |
| control-panel partners | partner/store eligibility control and readiness handoff into marketing | `dsh/frontend/control-panel/partners/ControlPanelDshPartnerApprovalsScreen.tsx` -> `dsh/frontend/control-panel/partners/DshPartnerPromotionEligibilityScreen.tsx` | route-backed partners section; queues `offer-approval`, `partner-review`, and `marketing-review`; dedicated eligibility section now shows approval, operational, category, and featured readiness before marketing handoff | PARTNER_ELIGIBILITY_CONTROL | CANDIDATE_SCREEN | keep the intake hub and eligibility section together; prove runtime and persistence later |
| control-panel marketing | campaign/banner/growth control, ordering, publish state, and destination metadata | `dsh/frontend/control-panel/marketing/ControlPanelDshMarketingScreen.tsx`; `dsh/frontend/control-panel/marketing/GrowthCommandDeckScreen.tsx`; `dsh/frontend/control-panel/marketing/BannersCommandDeckScreen.tsx` | marketing section route is proven; growth deck edits campaign/promo/source/routeTarget fields, banner deck edits `title`, `subtitle`, `audience`, `status`, `actionType`, `actionTarget`, `actionExtra`, `imageUrl`, and `position` with live preview | MARKETING_CAMPAIGN_CONTROL | PROVEN_SCREEN | keep control-panel marketing as the proven governance surface until shared runtime authority is proven |
| app-client | final banner/promo display and destination routing | `dsh/frontend/app-client/home/screens/DshHomeGetScreen.tsx` | `BannerCarousel` is rendered from `bannerItems`; promos route through `resolveBannerPress`; `DshSurfaceHost.tsx` injects `resolvePublishedHomePromos()` | CLIENT_DISPLAY | PROVEN_SCREEN | prove authoritative runtime source later; no implementation in this phase |
| dsh/media-fixtures | seed media reference source only, not runtime authority | `dsh/media-fixtures/assets/seed/dsh/banners` | seed banner assets plus `mediaKey` references in shared banner store; resolved through local image helpers only | MEDIA_FIXTURE_SOURCE | PROVEN_SCREEN | keep media mapping documented as seed-only; no implementation in this phase |
