...
- donor DSH service scope spans five real surfaces: APP_CLIENT, APP_CAPTAIN, APP_PARTNER, APP_FIELD, and CONTROL_PANEL
...
- donor CONTROL_PANEL route evidence proves a real internal route cluster for DSH, especially for proxy-style handling and internal operations

## Controlled Inferences

- most donor CONTROL_PANEL routing is an umbrella visibility pattern rather than clean internal ownership for every DSH action family
...
- do not place every DSH family in `control-panel` just because donor CONTROL_PANEL exposed broad operational pages
