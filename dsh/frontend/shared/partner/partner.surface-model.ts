...
// Authority: dsh/frontend/shared/partner — partner surface state model and hook.
...
import { usePartnerOrdersRuntime } from './use-partner-orders-runtime';
import {
  buildSupportCommandContextFromOperationalFlow,
  buildSupportCommandContextFromSupportRoute,
  defaultSupportCommandContext,
} from '../support/support.partner-context';
import { isCommandCenterInlineManagedRoute } from '../support/support.partner-policies';
...
export function useDshPartnerSurfaceModel(
  ...
  const state: DshPartnerSurfaceState = {
    route, storeScopeVisible, accountHubSection, ordersSearchMode,
    selectedStoreScopeId, editingProductId, activeOrderId, supportNav,
  };

  const actions: DshPartnerSurfaceActions = {
    setRoute, setStoreScopeVisible, setAccountHubSection, setOrdersSearchMode,
    setSelectedStoreScopeId, setEditingProductId, setActiveOrderId,
    setSelectedSupportScreen, setSupportCommandContext, handleOperationalFlowNavigation,
    openOrdersBoard, openOrdersSearch, openAccountHub, goBackToHub,
    openSupportDirectory, returnToSupportDirectory, openInventoryManagement,
    openStoreCourier, openWalletHub, openStoreScope, openSupportScreen,
    handleMarkReady, handleHardwareBackPress,
  };

  return {
    state, actions, selectedStoreScope, runtimePartnerProfile,
    partnerOrdersState, partnerOrders, deliveryOpsSummary, isCommandCenterInline,
  };
}
