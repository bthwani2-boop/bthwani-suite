...
| Giant screen candidates | 5 | `DshHomeGetScreen`, `DshStoreGetScreen`, `DshCartUnifiedScreen`, `DshPartnerHubSurface`, `DshCaptainOrdersScreen` |
...
| Surface | File | Bytes | Consumed / Referenced | Proven host or reference | Candidate flow | Related / mirror surfaces | Data source type | State / signal markers | Risk | Decision / next action |
| --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- | --- |
| app-partner | `dsh/frontend/app-partner/DshPartnerSurface.tsx` | 43908 | ACTIVE_CONSUMED | partner shell/compat bridge consumes partner hub surface | Partner hub root | app-partner primary; control-panel partners and catalogs mirrors | local UI models + seeded cards | queue/section states | GIANT_SCREEN_CANDIDATE, no backend proof | keep; package with partner preparation |
| app-partner | `dsh/frontend/app-partner/DshPartnerEntryScreen.tsx` | 4761 | ACTIVE_ENTRY_CANDIDATE | `screens/PartnerEntryScreen.tsx` wraps the root entry implementation consumed by the partner shell bridge | Partner entry | app-partner only; entry root without confirmed mirror | UI-only | entry CTA states | entry exists, no closure proof | keep as root entry |
| app-partner | `dsh/frontend/app-partner/screens/InventoryCatalogScreen.tsx` | 956 | ACTIVE_CONSUMED | partner shell/compat bridge imports `InventoryCatalogScreen` directly | Partner inventory / catalog handoff | app-partner primary; control-panel partners and catalogs mirrors | props-driven wrapper over shared section | ready/back-navigation states | no inventory backend proof | keep; package with partner console |
...
| 7 | app-partner | Partner hub board | `DshPartnerSurface.tsx` | السطح الرئيسي للشريك | local models |
| 8 | app-partner | Partner orders workspace | `DshPartnerOrdersScreen.tsx` | إدارة الطلبات والتحضير | local models |
| 9 | app-partner | Partner inventory block | `InventoryCatalogScreen.tsx` | readiness/catalog handoff | props |
...
| 14 | Partner Hub / Preparation | app-partner + control-panel partners/catalogs | `DshPartnerSurface.tsx`, `DshPartnerOrdersScreen.tsx`, `ControlPanelDshPartnerApprovalsScreen.tsx` | local models + local queue/workflow data | partner intake/approval APIs not ready | partner state machine and audit trail not closed | role/auth-bound surface; no direct WLT block | ACTIVE_NOT_CLOSED | package 05: partner hub/preparation |
...
|---|---|---|---|---|---|---|
| app-partner | OFFER_SOURCE | `dsh/frontend/app-partner/DshPartnerSurface.tsx` -> analytics section -> PromotionIntentPanel | promotion intent, candidate selection, draft title/note, and submit/request CTA live inside the analytics section | CANDIDATE_SCREEN | CANDIDATE_SCREEN | keep as embedded hub context; route/runtime persistence remains unproven |
