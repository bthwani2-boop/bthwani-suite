...
|---|---|---|---|---|---|---|---|---|---|
| `DSH-SAPI-P014-01` | `app-client` | `/app-client/discovery` | `HomeScreen.tsx`; `SearchScreen.tsx`; `StoreScreen.tsx` | open a destination, store, or category | `loading`, `empty`, `error`, `success`, `offline` | app-client / GET /stores edge proof only; not full cross-surface Slice 001 closure | no WLT boundary yet | pending cross-surface proof for Slice 001 | `visual captured / VISUAL_PASS; runtime proof deferred` |
...
| `DSH-SAPI-P014-05` | `app-partner` | `/app-partner/inventory` | `InventoryCatalogScreen.tsx` | update readiness and publishing visibility | `loading`, `empty`, `error`, `success`, `offline` | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` | no WLT boundary | frontend UI screen not yet connected to the backend API | update matrix after E2E live DB curl or frontend connection |
...
| `DSH-SAPI-P014-12` | `backend/domain/openapi` | `dsh/backend`; `dsh/domain`; `dsh/dsh.openapi.yaml` | service scaffold only | none accepted yet | `BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` | auth, runtime, persistence, and observability verified via unit tests | E2E Live DB curl validation and frontend UI integration verify proof | update matrix after E2E live DB curl or frontend connection |

Exit gate:
`BACKEND_CONTRACT_AND_REPOSITORY_PROVEN__LIVE_E2E_PENDING` is passed for `DSH-SAPI-P014-01`, `DSH-SAPI-P014-05`, `DSH-SAPI-P014-10`, and `DSH-SAPI-P014-12` after implementation and verification of the visibility gate endpoints.
