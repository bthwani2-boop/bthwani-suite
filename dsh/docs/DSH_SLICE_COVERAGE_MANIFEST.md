...
| `DSH-SLICE-003A` | J-003 | Client reviews cart and confirms serviceability lock | client | app-client (cart) | shared serviceability model | app-captain (N/A: not yet assigned); WLT (N/A: payment not started); app-partner (N/A: J-001); app-field (N/A: J-006); control-panel (N/A: no CP action at cart step) | view cart; remove item; proceed to checkout | loading, empty, error, blocked, retry, serviceable, not_serviceable | preview/local-state | CONTRACT_DESIGNED (dsh.openapi.yaml GET /cart/serviceability + backend checkout_handler.go implemented) — BLOCKED_BY_AUTH: BearerAuth middleware pending; currently DEV_ONLY X-Client-Id header; Auth SSoT = root auth.openapi.yaml GET /auth/session (dsh/auth.openapi.yaml removed — was duplicate /auth/me contract) | WLT starts at payment intent (003C) | client auth — PRIMARY BLOCKER: BearerAuth not wired; X-Client-Id DEV_ONLY | yes — 7 states required | NEEDS_RUNTIME_EVIDENCE — blocked on BearerAuth | BearerAuth wired + auth endpoint conflict resolved + CartScreen wired + runtime proof + visual proof (all 7 states) | BLOCKED_WITH_REASON — BearerAuth + auth endpoint SSoT unresolved; slice file fully upgraded (DSH_SLICE_003A_CART_SERVICEABILITY_BLOCKED_CLOSURE-20260604-174100) | Wire BearerAuth against auth.openapi.yaml GET /auth/session; do NOT start DSH-SLICE-003B until this slice PASS |
| `DSH-SLICE-003B` | J-003 | Client submits checkout intent; system reserves items and prepares order session | client | app-client / DshCheckoutIntentScreen | DSH backend checkout session service | app-captain (N/A); app-partner (N/A: intake is 003D); app-field (N/A: J-006); control-panel (N/A: no operator action) | confirm checkout; cancel; change address | loading, address_entry, intent_created, intent_failed, blocked, error | preview/local-state | CONTRACT_DESIGNED (dsh.openapi.yaml POST /checkout/intent + backend checkout_handler.go implemented) — BLOCKED_BY_AUTH: BearerAuth middleware pending; DEV_ONLY X-Client-Id; 003A PASS required | WLT receives session token at 003C | client auth (upstream from 003A) — DEV_ONLY X-Client-Id currently | yes — 6 states required | NEEDS_RUNTIME_EVIDENCE — blocked on 003A + BearerAuth | 003A PASS + BearerAuth wired + DshCheckoutIntentScreen wired + runtime + visual proof | BLOCKED_WITH_REASON — 003A PASS + BearerAuth required; slice fully upgraded (DSH_SLICE_003B_003E_BLOCKED_COMPLIANCE_CLOSURE-20260604-174700) | Await 003A PASS; do NOT start 003C until 003B PASS |
| `DSH-SLICE-003C` | J-003 | Payment executed via WLT; DSH receives payment-confirmed callback and stores reference ID only | WLT (primary); DSH backend (callback) | WLT (external boundary); app-client (WltBoundaryBanner) | DSH backend callback handler; control-panel finance (read-only) | app-captain (N/A); app-partner (N/A: intake is 003D); app-field (N/A: J-006); WLT wallet mutation (excluded: WLT owns entirely) | Pay via WLT; (auto) receive callback; retry on WLT failure | awaiting_wlt_confirmation, payment_confirmed, payment_failed, loading, error | WLT-owned (payment); DSH stores reference ID only | CONTRACT_DESIGNED (dsh.openapi.yaml POST /checkout/payment-callback) — SECURITY_INCOMPLETE: security:[] DEV_ONLY; WLT service auth/HMAC/Idempotency-Key/replay-protection not specified; ARCHITECTURAL_CONFLICT: dsh.openapi.yaml uses callback-primary but wlt.openapi.yaml describes DSH polling GET /wlt/dsh/client/payment-sessions/{sessionId} — polling vs callback must be resolved before PASS; 003B PASS required | Full WLT ownership — DSH callback only; no finance mutation; requested_amount_minor_units in DB is non-authoritative snapshot only | WLT-managed auth — UNRESOLVED: callback security scheme not yet confirmed with WLT | yes — WltBoundaryBanner states required | NEEDS_WLT_RUNTIME_PROOF | WLT callback spec published + polling vs callback resolved + WLT service auth/HMAC/idempotency implemented + 003B PASS + WLT E2E proven + DSH callback proven | BLOCKED_WITH_REASON — WLT runtime proof + callback security + polling/callback architectural conflict + 003B PASS required; slice fully upgraded (DSH_SLICE_003B_003E_BLOCKED_COMPLIANCE_CLOSURE-20260604-174700) | Resolve WLT polling vs callback architecture; add WLT service auth + idempotency to POST /checkout/payment-callback; await 003B PASS; do NOT start 003D or 003E until 003C PASS |
| `DSH-SLICE-003D` | J-003 | DSH creates order record on WLT payment confirmation; hands off to partner + ops monitor | DSH backend (automated); app-partner; control-panel ops | DSH backend / order service | app-partner (intake); control-panel ops monitor | app-captain (N/A: assigned in J-005); app-field (N/A: J-006); WLT (excluded: already provided payment confirmation) | (auto) create order; view order (client); view new order (partner) | payment_confirmed, order_CREATED, order_creation_failed, notification_sent, loading, error | DSH backend domain (order record); WLT payment reference (read-only) | BLOCKED — POST /orders not designed; requires 003B + 003C PASS | DSH reads WLT payment reference ID only; no DSH finance mutation | DSH internal + partner + operator auth | yes — client confirmation + partner intake + CP ops monitor | NEEDS_RUNTIME_EVIDENCE | 003B + 003C PASS + POST /orders designed + backend + DB migration + partner notification proven + runtime proof | BLOCKED_WITH_REASON — depends on 003B + 003C PASS; slice fully upgraded (DSH_SLICE_003D_FULL_UNIVERSAL_CLOSURE-20260604-181500) | Await 003B + 003C PASS; do NOT start J-004 partner lifecycle until 003D PASS |
| `DSH-SLICE-003E` | J-003 | Client receives failure feedback + recovery path when WLT payment fails; cart preserved | client | app-client / CheckoutFailureScreen | DSH backend (failure callback); app-client cart (preserved) | app-captain (N/A); app-partner (N/A); app-field (N/A: J-006); control-panel (NOT_APPLICABLE at this step — escalation in J-004) | retry payment; cancel checkout; contact support | payment_failed, retry_in_progress, cancelled, cart_preserved, loading, error | WLT-owned (failure reason); DSH stores failure event | CONTRACT_DESIGNED (dsh.openapi.yaml DELETE /checkout/intent/{id} + backend checkout_handler.go implemented) — BLOCKED_BY_WLT: WLT failure error spec not published; 003C PASS required | WLT owns failure reason; DSH displays only; no finance mutation | client auth (session from 003B) | yes — 6 states + cart preserved state | NEEDS_RUNTIME_EVIDENCE | 003C PASS + WLT error spec + CheckoutFailureScreen built + registered + runtime + visual proof | BLOCKED_WITH_REASON — 003C PASS + WLT error spec required; REQUIRED_ADDITION: CheckoutFailureScreen not yet in screen registry; slice fully upgraded (DSH_SLICE_003E_FULL_UNIVERSAL_CLOSURE-20260604-182000) | Await 003C PASS + WLT error spec; build + register CheckoutFailureScreen; J-003 NOT closed until 003E PASS |
