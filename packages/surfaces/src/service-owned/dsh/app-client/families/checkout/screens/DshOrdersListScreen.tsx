  ...
  return (
    <BthMobileScrollView padding={4} gap={3}>
      <BthBox gap={2}>
        ...
        <BthText role="bodySm" tone="muted">Returning-user list pattern with lightweight search and one-tap status filters.</BthText>
      </BthBox>

      <BthSurface tone="inset" gap={3}>
        <BthSearchField label="Find order" value={query} onChangeText={onQueryChange} hint="Search by title, status, or ETA to narrow the queue fast." />
        <BthTabs
          items={[
      ...
      {onOpenOrder && latestOrderId ? (
        <BthBox layoutDirection="row" justify="space-between" align="center">
          <BthText role="caption" tone="muted">Showing {filteredItems.length} {getOrdersListFilterLabel(activeFilter).toLowerCase()} orders</BthText>
          <BthButton label="Open latest" tone="ghost" onPress={() => onOpenOrder(latestOrderId)} />
        </BthBox>
      ) : null}

      <BthSurface tone="raised" gap={3}>
        <BthSectionHeader title="Recent orders" subtitle="Open order should take the user to the next clear action with one tap." count={filteredItems.length} />
        <BthBox gap={2}>
          {filteredItems.map((item) => (
            <BthListItem key={item.id} title={item.title} subtitle={item.subtitle} meta={item.meta} badgeLabel={item.statusLabel} onPress={() => onOpenOrder?.(item.id)} />
          ))}
        </BthBox>
      </BthSurface>
    </BthMobileScrollView>
