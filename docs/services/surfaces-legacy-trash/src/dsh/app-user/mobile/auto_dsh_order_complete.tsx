// Auto-generated screen for dsh_order_complete
// Surface: app-client | Service: dsh | Operation: POST /api/dsh/orders/:orderId/complete
...
export const auto_dsh_order_complete: React.FC<Props> = ({ onNavigate, navigation, route }) => {
  ...
  const submit = useCallback(async () => {
    ...
    setState('loading');
    try {
      const url = `${getBaseUrl()}/api/dsh/orders/${encodeURIComponent(id)}/complete`;
  ...
  if (state === 'loading') {
    return (
      <ScreenWrapper
        state="loading"
        loadingMessage={t('dsh.app-client.mobile.auto_dsh_order_complete.loadingMessage')}
        screenName="auto_dsh_order_complete"
        operationName="dsh_order_complete"
      />
    );
  }
  ...
  if (state === 'error') {
    return (
      <ScreenWrapper
        state="error"
        errorMessage={t('dsh.app-client.mobile.auto_dsh_order_complete.errorMessage')}
  ...
  return (
    <ScreenWrapper state="content">
      <View style={styles.container}>
        <Text style={[styles.title, textAlignStart]}>إتمام طلب</Text>
        <Text style={[styles.subtitle, textAlignStart]}>معرف الطلب (orderId)</Text>
        <TextInput
          style={[styles.input, textAlignStart]}
          value={orderId}
          onChangeText={setOrderId}
          placeholder={t('dsh.app-client.mobile.auto_dsh_order_complete.orderIdPlaceholder')}
        ...
        <TouchableOpacity style={styles.primaryButton} onPress={() => void submit()} disabled={!orderId.trim()}>
          <Text style={styles.primaryButtonText}>إتمام</Text>
        </TouchableOpacity>
        {success && (
          <View style={styles.result}>
            <Text style={[styles.resultText, textAlignStart]}>تم إتمام الطلب بنجاح</Text>
          </View>
        )}
...
export default auto_dsh_order_complete;
