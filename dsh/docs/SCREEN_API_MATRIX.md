...
| `DSH-SAPI-P014-02` | `app-client` | `/app-client/cart` | `CartScreen.tsx`; `DshCheckoutIntentScreen.tsx` | review cart and hand off payment choice | `loading`, `error`, `blocked`, `retry` | `BLOCKED_BY_WLT/AUTH` | WLT owns payment decision; auth is still unproven | all J-003 slices (003A–003E) compliance-upgraded (DSH_SLICE_003A_FULL_UNIVERSAL_CLOSURE-20260604-175500; DSH_SLICE_003B_003E_BLOCKED_COMPLIANCE_CLOSURE-20260604-174700); GET /cart/serviceability not in OpenAPI — not designed; REQUIRED_ADDITION: CartScreen registry must add 'retry' state; REQUIRED_ADDITION: CheckoutFailureScreen not yet built or registered | keep blocked until WLT/auth runtime proof exists; then design GET /cart/serviceability; forward-only gate: 003A PASS before 003B |
